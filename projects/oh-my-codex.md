...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | oh-my-codex 是构建于 Codex CLI 之上的多 Agent 编排层，职责包括角色化任务编排、持久化状态、可复用工作流和 MCP 外部工具接入；不替代 Codex CLI，也不是独立 Agent 框架。 | 仅明确 Codex CLI、Staged Team Pipeline、Role Keywords、OMX Skills、`.omx/` 和 MCP Server；入口渠道、权限控制、部署形态与具体协议未明确。 |
| 主路径 | 使用者或上游系统进入编排层；Staged Team Pipeline 按 Planner、Coder、Reviewer 等角色分阶段执行，通过 Codex CLI 使用模型能力，并经 MCP 接入外部工具和数据源；计划、日志、记忆与运行时状态写入 `.omx/`。 | 角色仅为示例；CLI 交互细节、MCP 传输方式、状态回写粒度及任务失败处理均未给出。 |
| 关键权衡 | 采用薄编排层可降低对 Codex 内部实现的依赖并复用编排逻辑，但无法深度控制底层 Agent；扩展 MCP 与角色工作流还会增加权限、可观测性、状态一致性和 Codex 接口兼容风险。 | 档案未提供权限模型、日志审计、SLO、成本指标、故障恢复或兼容适配机制，均待源码与文档核验。 |
| 最小 PoC | 选择一个可拆分且可验收的编码任务，验证 Planner、Coder、Reviewer 分阶段协作、一个 OMX Skill 的复用、`.omx/` 跨会话状态，以及一个只读 MCP 数据源；验收应覆盖接口变化、工具权限、日志审计、退出方式和失败回退。 | PoC 只能验证档案已声明能力；是否支持多模型、具体只读工具、隔离与回滚机制均为“待核验”，不得在 PoC 设计中预设存在。 |
...
```mermaid
flowchart LR
    U[使用者或上游系统] --> C[oh-my-codex 项目编排与运行时]
    C --> P[Staged Team Pipeline<br/>Planner、Coder、Reviewer 角色待核验]
    C --> S[.omx/<br/>计划、日志、记忆、运行时状态]
    C --> M[Codex CLI]
    M --> L[模型或推理服务<br/>供应商与协议待核验]
    C --> T[MCP Server]
    T --> X[外部工具与数据源]
    C --> R[权限、可观测性与 Codex 兼容性风险<br/>控制措施待核验]
```
