...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 定位为 AI Agent 编排框架，向 LLM 后端（OpenAI、Anthropic、开源模型）与外部工具/数据源开放 | 后端名称与外部集成面依据档案"多 LLM 后端支持"；具体协议、SDK 版本、传输层未在档案中给出，待核验 |
| 主路径 | 规划器 → 执行器 → 记忆/工具调用 → Ohmo 内置助手回路 | 架构抽象自档案"规划器、执行器、记忆、工具"四模块描述；具体消息格式、状态机与会话回写未公开，待核验 |
| 关键权衡 | 学术可实验性（可替换模块、可复现）优先于工程封装与生产可观测性 | 判断基于档案"设计上强调可实验性和可复现性"以及"风险/局限"段对工程成熟度的提醒；缺乏量化运维指标 |
| 最小 PoC | 单一 Ohmo 通道 + 单一 LLM 后端 + 最小工具集 + 启用内置评估框架收集日志 | PoC 范围依据档案"内置个人助手 Ohmo""多 LLM 后端""Agent 评估框架"三条亮点；安全、计费、退出路径等验收项需自行补充 |
...
flowchart LR
    ...
    I --> C[编排与运行时 含规划器 执行器]
    C --> M[模型后端 OpenAI Anthropic 开源模型 待核验]
    C --> T[工具与外部系统 待核验]
    C --> Mem[记忆模块 待核验]
    C --> E[内置评估框架]
    C --> Ohmo[内置个人助手 Ohmo]
