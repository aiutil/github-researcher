...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 多市场个人交易 Agent 系统，Python 实现，承担多 LLM Provider（OpenAI/Claude/Kimi/GLM/Codex）与多市场（美股/港股/A股/印度 NSE·BSE）之间的编排层，封装 460 个 Alpha 因子与多 Agent 辩论（Swarm）；具体入口形态、鉴权与持久化实现以源码核验为准。 | 档案给出分类、语言、标签与组件名称；未提供 API 协议、数据库选型、部署拓扑细节。 |
| 主路径 | 客户端请求 → 入口与身份边界 → 项目核心编排/运行时（含多 Agent Swarm 辩论） → 模型推理 + 市场特定交易规则引擎与数据源 → 决策回写到会话/状态/审计；交易执行链路具体形态（手动、半自动、自动）档案未直接描述。 | 仅档案明列的组件名可入图；执行是否触达券商通道、是否含实盘下单未在档案中确认。 |
| 关键权衡 | 多市场广度 vs 单市场深度（4 市场覆盖 vs 单市场专业度）、Agent 自治速度 vs 风控拦截能力（档案承认后果不可逆却未列具体止损/仓位/异常拦截实现）、LLM 多 Provider 灵活性 vs token 成本与供应商耦合、学术实验室信任 vs 生产可审计性。 | 权衡判断基于档案风险段与定位比较；具体风控模块、token 预算控制、可解释性实现均待核验。 |
| 最小 PoC | 单一市场（如美股）、单一 LLM Provider、最小工具权限、可审计日志四项约束下验证；验收须包含决策可追溯、成本/SLO 边界与退出路径；与同类（ai-hedge-fund / TradingAgents）做特性差异回归。 | 档案建议但未给出 Docker 内部端口、镜像签名、依赖矩阵；最小 PoC 所需参数以官方仓库 README/Dockerfile 核验为准。 |
...
```mermaid
flowchart LR
    U[使用者或上游系统] --> I[入口与身份边界<br/>FastAPI API Server Docker 一键部署 待核验]
    I --> C[项目核心编排与运行时<br/>Python]
    C --> M[模型或推理服务<br/>OpenAI Claude Kimi GLM Codex]
    C --> T[工具与外部系统<br/>460 Alpha 因子 美股 港股 A股 印度 NSE BSE 数据源 待核验]
    C --> R[多市场交易规则引擎<br/>T+1 涨跌停 费用结构 待核验]
    C --> S[会话 状态 审计<br/>API Server 模块化重构 待核验]
    M --> C
    R --> C
    T --> C
    S --> A[风控与可解释性边界<br/>止损 仓位 异常拦截 实盘通道 待核验]
```
