...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 入口渠道（PR 源）、LLM 供应商（OpenAI/Anthropic 兼容）、审查规则/工具数据之间的编排层；档案未证实的具体协议与部署形态 | 仅基于档案描述的"混合架构 + 行级评论 + 多语言规则集 + Agent Skill 扩展"，未给出源码级实现 |
| 主路径 | PR diff 与仓库上下文 → 确定性规则管线（AST + 模式匹配）→ LLM Agent 补充上下文问题 → 行级评论输出 | 档案明确给出该顺序，但具体编排、并发模型与持久化方式"待核验" |
| 关键权衡 | 确定性规则（精确、低成本、覆盖面有限）vs LLM Agent（智能、上下文感知、成本/延迟高、幻觉风险）的分工与边界 | 档案明确指出此权衡；具体误报率、成本数据未披露 |
| 最小 PoC | 单仓库单语言（优先 Java 或 Go）、单一 LLM 供应商、最小规则集、可审计日志，验收项含误报率、PR 延迟、LLM 成本与退出路径 | 档案给出"先在单一渠道、最小工具权限和可审计日志下验证"建议，未给出基准数字 |
...
```mermaid
flowchart LR
    PR[PR 与仓库上下文] --> IN[入口与权限边界 待核验]
    IN --> ORCH[项目编排与运行时]
    ORCH --> RULES[确定性规则管线 AST 与模式匹配 内置 Java Python Go JavaScript 等]
    ORCH --> LLM[LLM Agent 上下文与逻辑审查 OpenAI 或 Anthropic 兼容 API 待核验]
    RULES --> OUT[行级评论输出]
    LLM --> OUT
    ORCH --> STATE[会话 状态 审计 待核验]
    ORCH --> SKILL[Agent Skill 自定义规则扩展 待核验]
    OUT --> CI[CI/CD 审查节点]
    OUT --> HUMAN[人工确认 待核验]
```
