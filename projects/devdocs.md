...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | devdocs 是一份"数据产品"：上游 500+ 官方文档源、中游 Ruby 索引/检索核心、下游 Web/PWA 消费应用；边界由项目语言（Ruby）、许可（MPL-2.0）与离线/PWA 标签划定。 | 组件命名属档案"关键技术亮点"抽象，未在档案中点名具体服务/进程名；具体模块须读源码确认。 |
| 主路径 | 外部文档源 → 采集/规范化 → 核心索引 → 离线缓存（PWA/Service Worker）→ 浏览器/PWA 消费者，附键盘快捷键与暗色主题作为交互层。 | "离线 + PWA、键盘优先、聚合搜索、暗色模式"由档案显式列出；底层检索引擎（如是否引入 ElasticSearch/Lunr 等）档案未证。 |
| 关键权衡 | 新鲜度（文档版本滞后）vs 覆盖广度（500+ 源）、离线可用性 vs 数据体量、跨栈统一搜索 vs 单栈 ranking 准确性、Ruby 单体维护人力 vs 改造成本。 | 风险段已点明"Ruby 单体""ranking 不准""同步延迟"；AI 替代压力来自档案"风险/局限"。 |
| 最小 PoC | 选 1–2 个目标文档集（建议 MDN + React）在本地完成索引构建 + 离线缓存命中验证，再以 `/` 搜索、`j/k` 导航、`c` 复制代码三项快捷键做交互验收；归档验收包含暗色模式与断网可用两项。 | PoC 步骤基于档案"关键技术亮点"功能点拼接；索引规模、构建时长、Service Worker 缓存策略细节均待核验。 |
...
```mermaid
flowchart LR
    A[500+ 官方文档源 MDN/React/Vue/Rust/K8s 等 待核验抓取协议] --> B[采集 解析 规范化 Ruby 实现 待核验]
    B --> C[聚合索引与搜索 跨栈统一 ranking 准确性待核验]
    C --> D[离线缓存 PWA Service Worker 待核验持久化方案]
    D --> E[Web/PWA 消费者 键盘优先 暗色模式]
    C --> F[质量 观测 治理 文档同步延迟 风险边界]
    E --> G[状态/控制边界 断网降级 与 AI 助手互为兜底]
```
