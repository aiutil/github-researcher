...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 定位为 Agent Operating System，统一运行时，介于入口/上游系统与模型/工具之间，承担编排、调度、权限隔离与可观测性职责 | 基于 tags（agent-framework/ai-agents/operating-system/mcp）与档案描述；未给出具体入口协议、模型供应商适配清单 |
| 主路径 | 请求经入口与身份边界进入 Rust 运行时，由编排层调度后调用模型推理与 MCP 工具，结果回写会话/状态/审计，Agent 以"进程"为单位被管理 | 依据"Agent 即进程 / MCP 原生集成 / 资源调度 / 可观测性"四点；具体 IPC、调度算法、Token 计量实现未公开 |
| 关键权衡 | 在"抢占 Agent OS 命名先机"的速度与权限模型、资源配额、审计日志等生产级能力深度之间的取舍；同时受限于 4 个月项目历史与单一团队维护能力 | 来自档案风险段（概念超前、生态冷启动、Rust 门槛、RightNow-AI 维护可持续性）；不含实际性能基准 |
| 最小 PoC | 单一入口渠道 + 最小 MCP 工具集合 + 受限权限与审计开启，在沙箱中验证 Agent 生命周期（启动/暂停/终止）、资源配额与故障隔离，再决定是否扩大接入 | 来自档案采用建议与"先做最小 PoC"原则；具体部署形态、硬件要求、SLO 未在档案中给出 |
...
flowchart LR
  ...
  I --> C[openfang 编排与运行时 Rust]
  C --> M[模型或推理服务]
  C --> T[MCP 工具与外部系统]
  C --> S[会话 状态 审计]
  M --> C
  T --> C
  C --> P[Agent 进程 调度 权限隔离 资源配额]
  P --> C
  C -. 失败回写 .-> S
  note[待核验 部署形态 持久化 调度算法]:::risk
  C -.- note
