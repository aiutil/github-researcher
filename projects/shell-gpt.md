...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | CLI 入口 `sgpt` 作为编排层，统一对接 OpenAI / Anthropic / Azure OpenAI / Ollama 等模型供应商，对外向 Shell 与 Unix 管道暴露 | 供应商列表、CLI 命令与管道用法见档案；具体协议、SDK 细节未在档案中给出，待核验 |
| 主路径 | 用户在终端发起 `sgpt` 调用 → 角色预设与缓存命中判断 → 调用模型 API → 必要时生成 Shell 命令或经管道回写 → REPL/会话与本地缓存回写 | REPL、缓存、`sgpt -s`、管道组合均有档案描述；持久化与缓存后端实现细节待核验 |
| 关键权衡 | 多模型可扩展性 vs 对各供应商 API 的耦合深度；轻量 CLI 体验 vs Shell 命令直接执行带来的权限与误执行风险；本地 Ollama 隐私优势 vs 本地模型使用门槛 | 权衡来源于档案定位描述；具体权限模型、沙箱与审计能力未在档案中说明，待核验 |
| 最小 PoC | 单渠道（OpenAI）＋单角色（Shell 命令生成 `sgpt -s`）＋本地缓存关闭＋日志全开，先验证命令生成的可用性与误执行风险，再扩展到 Anthropic/Ollama | PoC 范围由档案已声明能力（多模型、Shell 生成、管道、缓存）推导；具体安全/成本/SLO 指标档案未提供 |
...
```mermaid
flowchart LR
    U[使用者或上游系统] --> I[终端入口 sgpt 与角色预设]
    I --> C[项目编排与运行时]
    C --> M[模型供应商 OpenAI Anthropic Azure OpenAI Ollama 待核验]
    C --> T[Shell 执行与 Unix 管道]
    C --> S[REPL 会话 本地缓存 审计日志 待核验]
    M --> C
    T --> C
    S --> C
    T -.直接执行风险 待核验.-> U
