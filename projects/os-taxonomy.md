...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 数据产品型边界：公开课程标准（NGSS/Common Core/UK NC）作为外部输入，Marble 维护的 DAG+JSON Schema 作为内部核心资产，外部消费方仅通过开放 JSON 制品访问，不含 Marble 自有产品/推荐引擎 | 边界依据来自档案"架构启发"段落明确写明"商业价值在交互体验和个性化推荐引擎"，故二者应排除在核心边界外；具体运行时、协议、部署形态未披露 |
| 主路径 | 外部标准 → Marble 内部采集/规范化/Schema 校验（含 SHA-256 manifest）→ 1590 微主题 + 3221 依赖边的 DAG 制品 → 通过开放 JSON 制品交付给消费方（教育 AI / 自适应学习 / 辅导） | 节点数与边数及 manifest 校验属实于档案；处理/存储中间件细节未披露，标"待核验" |
| 关键权衡 | 数据质量与治理（Schema 强制 + 域聚类 + 中心性） vs 单一公司维护带来的版本治理与可持续性风险；标准化覆盖率（仅英语国家 K-12） vs 多地区适配扩展成本 | 权衡两侧均有档案直接文本支撑；社区治理模式、版本节奏在档案中明确标注为"不清晰" |
| 最小 PoC | 克隆制品 → 校验 manifest.json（SHA-256）→ 加载 DAG → 以单一微主题为种子遍历前置依赖 → 用评估提示模板渲染一次失败回退以验证链路 | 档案明确提供 manifest 校验、Schema 目录、mastery evidence/assessment prompt 字段；POC 中涉及的任何 API 协议、数据库或服务均未在档案中描述，不补写 |
...
```mermaid
flowchart LR
    A[外部课程标准 NGSS / Common Core / UK NC] --> B[采集 规范化 Schema 校验]
    B --> C[Marble 核心: 1590 微主题 + 3221 依赖边 DAG]
    C --> D[domain-clusters 183 个家长友好摘要]
    C --> E[manifest.json 含 SHA-256 校验]
    C --> F[开放 JSON 制品]
    F --> G[消费方: 教育 AI 辅导 / 自适应学习 / 待核验]
    C --> H{{治理边界: 单一公司维护 社区参与模式 待核验}}
```
