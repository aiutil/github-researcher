...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 研究对象是 ESP32-S3 上的离线语言模型推理系统，核心约束为 512KB SRAM、8MB PSRAM 与 16MB Flash；25M 参数表置于 Flash，纯计算参数位于 SRAM，输出头与工作内存位于 PSRAM。 | 档案未给出具体入口界面、模型文件格式、训练数据全貌、固件协议及持久化实现，相关组件均待源码核验。 |
| 主路径 | 已验证主路径为输入 token → 从 Flash 读取约 6 行 embedding、约 450B → 在 ESP32-S3 上进行 4-bit 量化模型推理 → 输出文本；端到端实测约 9.5 tok/s，零网络连接。 | 档案明确仅能生成 TinyStories 级别短故事，不支持问答、指令或编码；具体 tokenizer、采样策略和运行时 API 待核验。 |
| 关键权衡 | 核心取舍是将大部分模型参数保留在较慢的 Flash，以较少 SRAM 占用换取约 100 倍于 260K 参数模型的规模；模型能力仍受 TinyStories 训练范围限制。 | 约 9.5 tok/s 与内存占用是项目自述结果；未提供功耗、延迟分项、Flash 寿命、不同任务质量、PSRAM 带宽影响或跨硬件复现结果。 |
| 最小 PoC | 在 ESP32-S3 上复现 28.9M 参数、4-bit 量化后 14.9MB 的离线短故事生成，并记录可运行性、实际内存、token/s、输出能力边界及 Flash 参数读取量。 | 训练、量化、实验代码位于 `src/` 和 `experiments/`，结果见 `RESULTS.md`；构建环境、烧录方式、硬件版本和依赖版本需按源码核验。 |
...
```mermaid
flowchart LR
    A[输入 token] --> B[ESP32-S3 推理核心]
    B --> C[Flash：25M 参数 embedding 表]
    C --> B
    B --> D[PSRAM：输出头与工作内存]
    D --> E[输出：TinyStories 级短故事]
    F[离线边界：零网络连接] -.约束.-> A
    F -.约束.-> E
    G[能力风险：仅短故事生成，不支持问答、指令、编码] -.限制.-> E
    H[工程化与跨硬件复现待核验] -.风险.-> B
```
