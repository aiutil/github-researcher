...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 自托管 Agent 编排平台，作为 GitHub Issue/外部使用者与 20+ 编码 Agent 之间的中间层，含 Agent 抽象、任务编排、沙箱隔离与审计 | 组件名称来自档案"关键技术亮点"；协议、API 形态、部署细节未在档案中证实 |
| 主路径 | Issue/任务 → 任务分配与编排 → 适配后的 Agent 执行（含沙箱） → 进度回写与 PR/复合输出 → 审计与状态持久化 | 流程依据档案"Issue 驱动"与"复合输出"描述；具体消息协议与存储未注明 |
| 关键权衡 | 多 Agent 适配广度（20+ Agent）vs 单 Agent 稳定耦合；团队协作上下文共享 vs 沙箱隔离；自托管数据本地化 vs Agent 厂商 API/行为变更的维护成本 | "厂商依赖""Agent 间通信复杂性"由档案明确指出；性能/SLO 数据缺失 |
| 最小 PoC | 选 1 个编码 Agent（如 Claude Code）+ 单一 GitHub 仓库 + 1 类任务，验证 Issue→Agent→PR 主路径、沙箱隔离、可审计日志与 Agent 升级时的回归 | 档案未给出官方示例或安装指引，部署细节需源码核验 |
...
```mermaid
flowchart LR
  U[使用者/团队] --> I[入口与身份边界<br/>GitHub Issue 等渠道]
  I --> C[任务编排与运行时<br/>串行/并行/条件分支]
  C --> A[Agent 抽象层<br/>适配 Claude Code/Codex/Cursor 等 20+ Agent]
  A --> S1[沙箱隔离环境<br/>每 Agent 独立运行]
  S1 --> O[复合输出与冲突解决<br/>PR 创建]
  O --> R[进度追踪与状态回写]
  R --> C
  C --> AU[审计/可观测性边界<br/>待核验：日志与指标方案]
  EXT[外部 Agent 厂商<br/>API 与行为可能变化] -.适配维护.-> A
```
