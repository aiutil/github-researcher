...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | autoresearch 是单 GPU 上运行 nanochat 训练实验的编排层，介于使用者与 nanochat 训练流程之间 | 仅 Python 语言、nanochat 示范场景在档案内明确；具体入口协议、依赖服务未在档案内描述 |
| 主路径 | 使用者提交实验意图 → 编排层调度 → nanochat 训练执行 → 结果回写到记录/日志 | 主路径基于"自动实验编排、参数搜索、结果记录"与 Nanochat 训练作为示范场景两处描述拼接；具体编排实现未核验 |
| 关键权衡 | 在单 GPU 资源约束下，实验广度（多组消融/超参）与每组实验深度（步数/模型规模）的取舍 | "单 GPU 即可运行"与"研究自动化对 GPU 资源消耗大"在档案内直接矛盾，需以源码判定默认上限 |
| 最小 PoC | 在单 GPU 节点上固定 nanochat 基础配置运行一次端到端训练，记录编排耗时、GPU 占用与产出物 | 最小 PoC 仅依据"单 GPU 即可运行的轻量级研究自动化 + Nanochat 训练示范场景"；指标阈值、终止条件均"待核验" |
