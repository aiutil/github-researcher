...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 以 Claude Code / Codex CLI 为入口的编排层，负责把自然语言意图映射为 Lottie JSON 输出，外部依赖是 AI 编程助手运行时和模型服务 | 档案仅声明“Claude Code/Codex 集成、TypeScript、MIT、零 Issue”，未披露入口协议、模型供应商、鉴权方式 |
| 主路径 | 自然语言描述 → 入口/编排 → 调用模型与可用工具 → 产出 Lottie JSON → 浏览器预览与对话迭代回写 | “Lottie JSON 直接生成、预览和迭代”是档案原话；“状态回写”“工具调用”细节未在 README 证实 |
| 关键权衡 | 在 Lottie schema 强约束下提升生成复杂度，与品牌一致性、可解释性、可审计性之间的平衡；同时受 Claude Code/Codex 生态普及度影响 | 档案明列“生成质量上限、品牌一致性、依赖 AI 编程助手普及、TypeScript 但 0 Issue”风险；未给出性能或质量基准 |
| 最小 PoC | 在单一 Claude Code 或 Codex CLI 渠道、用最小工具权限与可审计日志运行一次“弹跳加载圆”描述到 Lottie JSON，并核验产物 schema 合法性 | 档案只声明入口是 CLI 类型的 Skill/Plugin；具体鉴权、日志、部署形态、扩展到 SVG/3D 等均为待核验项 |
...
```mermaid
flowchart LR
U[使用者] --> I[入口: Claude Code 或 Codex CLI]
I --> C[项目编排与运行时<br/>TypeScript]
C --> M[模型或推理服务<br/>供应商待核验]
C --> T[工具与外部系统<br/>范围待核验]
C --> J[Lottie JSON 生成<br/>schema 约束]
J --> P[浏览器预览与对话迭代]
C --> S[会话 状态 审计<br/>持久化待核验]
P -.微调回写.-> C
```
