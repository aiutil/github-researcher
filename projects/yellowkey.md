...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 信任边界位于"正常 Windows 系统镜像"与"WinRE 恢复镜像"之间：FsTx 组件同名但功能不同，WinRE 高权限组件成为加密卷前的最后一道信任闸口，绕过点就在此断层 | 仅基于档案"FsTx 在正常系统无此功能、仅 WinRE 存在"的描述，未核验 Microsoft 官方镜像 |
| 主路径 | 特制 USB（写入 System Volume Information\FsTx）→ 物理接入 WinRE（Shift+重启+Ctrl）→ FsTx 触发绕过 → 获得加密卷无限制 shell | 步骤引自档案"它解决的问题"小节，是否需要 EFI 写入权限、是否需 BitLocker 恢复密钥交互未在档案中说明 |
| 关键权衡 | 影响面（Win11/Server 2022/2025 全部 BitLocker 部署）vs. 攻击门槛（需物理/启动控制）vs. 修复不确定性（无补丁、无检测工具、原仓库 404） | "后门"还是"设计缺陷"档案明确标注"尚未独立验证"；CVE-2026-45585 仅见于衍生仓库命名 |
| 最小 PoC | 在受控隔离环境复刻一台 Win11 目标机，准备 USB 注入 System Volume Information\FsTx，按档案步骤进入 WinRE，观察是否拿到卷访问——验收项限定为"现象复现"，不外推为生产可用工具 | 复现细节（FsTx 内容、WinRE 启动时序、Ctrl 键时长）档案未给出，需原始 PoC（已 404）核验 |
...
```mermaid
flowchart TB
  A[特制 USB 存储设备<br/>写入 System Volume Information\FsTx] --> B[目标机 Windows 11<br/>Server 2022 或 2025]
  B --> C{进入 WinRE<br/>Shift+重启 + Ctrl<br/>触发方式待核验}
  C -- 触发 --> D[WinRE 镜像内 FsTx 组件<br/>档案指控：同名组件<br/>仅在 WinRE 具备绕过功能]
  D --> E[绕过 BitLocker 信任链<br/>获得加密卷无限制 shell]
  C -- 未触发 --> F[正常 Windows 系统<br/>FsTx 同名但无绕过功能<br/>档案描述为对比基线]
  D -. 原仓库 404 .-> G[威胁情报状态<br/>CVE-2026-45585 待官方核验<br/>Microsoft 补丁/公告待核验]
  E -. 攻击需要 .-> H[物理访问或 EFI 分区写入权限<br/>档案列为前提]
  G --> I[架构治理决策<br/>WinRE 审计工具/部署指南<br/>是否更新 待核验]
  H --> I
```
