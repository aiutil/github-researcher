...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 单一 C 代码库本地推理引擎，对外暴露 GPU 三后端（Metal/CUDA/ROCm）与 DeepSeek V4 模型文件，无 server/API 边界 | 档案仅声明三后端与纯 C 实现；具体入口形态（CLI 参数、socket/HTTP）未在档案中给出，待核验 |
| 主路径 | 模型文件 → C 加载/量化路径 → GPU 后端（Metal/CUDA/ROCm 任一）→ 本进程输出 token；无服务化中间层 | 档案描述"编译即用""无需 Docker/K8s""与 vLLM 服务化对比"，未给出请求-响应协议细节，待核验 |
| 关键权衡 | 极简 C 与零依赖 vs 三平台碎片化适配、量化收益 vs 精度损失、V4 专用优化 vs 单一模型绑定风险 | 权衡来自档案明示定位与局限章节；具体量化位宽、显存阈值、性能数字均未给出，待核验 |
| 最小 PoC | Apple Silicon（M-series）与单卡消费级 NVIDIA（具体型号待核验）上各跑一次 DeepSeek V4 Flash，验证加载、量化与首 token 延迟，验收项：编译命令、模型权重来源、显存占用、退出路径 | 档案支持"消费级硬件""Flash 轻量版""三后端"主张；具体硬件门槛、量化方案、token/s 基准未给出，待核验 |
...
```mermaid
flowchart LR
    U[使用者] --> I[本地 CLI 入口 待核验]
    I --> C[ds4 C 推理运行时]
    C --> Q[V4 专用量化路径 待核验]
    Q --> G{GPU 后端}
    G --> M1[Metal Apple Silicon]
    G --> M2[CUDA NVIDIA]
    G --> M3[ROCm AMD]
    C --> K[DeepSeek V4 模型文件 Flash / PRO]
    C --> O[本地 token 输出 无服务化层]
    C -.状态/风险.-> R[模型绑定 V4 单一 边缘 issue 421 待核验]
```
