...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 仅限 React 应用开发期的运行时检测：在浏览器/DevTools 中识别重渲染、给出修复建议；不涉及生产监控、跨框架与构建期优化 | 档案明确"专为 React 设计"、Vue/Svelte 不可用；接入方式含浏览器扩展/script/npm，但具体宿主与权限边界未在档案中证实 |
| 主路径 | React 应用挂载 → react-scan 注入运行时拦截渲染 → 组件树扫描与重渲染标记 → 给出 memo/useMemo/useCallback 类修复建议 | 档案描述了"扫描整个组件树、自动识别不必要重渲染、红框高亮、可执行修复建议"；底层拦截机制（fiber hook/patch）未在档案中说明 |
| 关键权衡 | 自动化建议 vs 建议正确性：把 Profiler 简化为"一键+可执行修复"，但建议基于经验规则，过度 memo 化可能反向损害性能；同时受 React Compiler 官方化和 React 19+ API 演进的威胁 | 档案明确点出"建议质量依赖经验规则"与"React 官方化/React Compiler 适配风险" |
| 最小 PoC | 在一个真实 React 项目中接入 react-scan，对比接入前后 Profiler 记录的渲染次数与耗时；验证建议项改写后是否真的降低重渲染；同时验证与 React Compiler 的兼容性以评估长期价值 | 档案仅提供能力声明与定位描述；具体性能数字、版本兼容矩阵、API 表面均需源码/Demo 核验 |
...
```mermaid
flowchart LR
    U[React 应用用户交互] --> R[React 组件树渲染]
    R --> S[react-scan 运行时拦截与扫描]
    S --> D{是否为不必要重渲染?}
    D -- 否 --> N[正常渲染路径]
    D -- 是 --> H[红框/边框可视化高亮问题组件]
    H --> A[给出 memo/useMemo/useCallback 类修复建议]
    A --> U
    S --> M[渲染耗时与频率量化]
    M --> Mnote["待核验:量化口径与采样方式"]
    A --> Anote["待核验:建议生成规则与覆盖边界"]
    S --> RC[外部边界:React 19+/React Compiler 适配风险]
    RC --> RCnote["待核验:具体兼容版本与降级策略"]
```
