...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | Rust + Tauri 桌面应用，按住全局快捷键触发，覆盖 macOS 与 Windows；本地 ASR + LLM 润色输出到系统剪贴板/输入法，定位为 Wispr Flow 的开源、BYOK、本地优先替代 | 平台仅声明 macOS/Windows，Linux 列为"后续观察"未交付；具体 ASR 引擎类型、LLM 调用方式未在档案中明确 |
| 主路径 | 按住快捷键 → 全局快捷键监听 → ASR 识别 → LLM 润色 → 通过剪贴板/输入法注入到目标应用光标处 | 剪贴板注入还是输入法层注入未在档案中区分；全局快捷键的具体键位、Tauri 系统级 API 使用细节未说明 |
| 关键权衡 | 体验与隐私 vs 润色质量的取舍：ASR 可走本地 Whisper（隐私优先），LLM 仍依赖云端 API（质量优先），构成混合架构；核心竞争力押注在 Prompt Engineering 而非模型层 | "LLM 可调用任何 API"为定性描述，BYOK 具体接入哪些供应商、是否支持本地 LLM 未在档案中证实 |
| 最小 PoC | 单 macOS 渠道、固定快捷键、单一 LLM 供应商 API Key、启用可审计日志，验证"按住说话→松开出文"端到端时延与润色可用性；将安全、成本、退出路径作为验收项后再扩到 Windows 与多供应商 | 345 stars/6 天增速与维护节奏仅作热度参考，不作为生产可用证据；功能迭代速度"不快"为定性判断 |
...
```mermaid
flowchart LR
    U[使用者按住快捷键] --> K[全局快捷键监听 Tauri 桌面端]
    K --> A[ASR 识别 待核验 本地 Whisper 或远程 API]
    A --> L[LLM 润色 BYOK 待核验 供应商未明]
    L --> P[Prompt Engineering 润色核心]
    P --> O[系统剪贴板或输入法 待核验 注入方式未明]
    O --> T[任意目标应用光标]
    K -. macOS / Windows .-> T
    L -. 待核验 隐私边界 .-> X[本地优先 vs 云端 LLM 混合架构]
</mermaid>
```
