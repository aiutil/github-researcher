...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 入口渠道、模型供应商、工具/数据源之间的编排层 | 基于项目分类、语言（公开资料未标注）、标签（llm, llm-inference, mistralai, transformer）及本档案引用的公开资料做的架构抽象，不替代源码审计。 |
...
flowchart LR
  ...
  M --> C
  T --> C
  A[仓库已归档 Archived 待核验替代方案] -.-> C
  E[vLLM TGI llama.cpp 等外部推理引擎] -.-> C
```
