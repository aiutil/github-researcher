...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | Lapce 应用位于用户/客户端与外部语言能力之间，内部由 Rust 自研的 Floem UI + 状态/领域核心 + WASI 插件运行时构成；LSP 复用 VS Code 生态语言服务器，SSH 用于远程编辑 | Floem、Druid/GPUI 三者并列见于档案；其余 UI 框架归属、是否同时存在多套 UI、核心模块切分需源码核验 |
| 主路径 | 用户编辑事件 → Floem UI 层 → 状态/领域核心 →（WASI 插件 / LSP / SSH 等服务接口）→ 外部语言服务器或远程主机 | 路径中的协议、线程模型、持久化与缓冲区结构均未在档案中给出，主路径只能画到职责级 |
| 关键权衡 | 用 Rust 自研 UI 框架与 WASI 插件换取极致性能与沙箱扩展，但承担生态冷启动、核心团队小、单 GUI 形态的可持续性风险 | Druid/GPUI 是否被实际启用、插件市场现状、商业/基金会支持均无证据 |
| 最小 PoC | 在本地用内置 Vim 模式编辑一个 Rust/中等规模项目，验证：冷启动延迟、LSP 实联调、远程 SSH 编辑可用、WASI 插件样例可加载与隔离；并以 Zed/VS Code 同场景做对照 | 启动时间、LSP 延迟、SSH 可靠性、可退出路径需自有基准，不引用未证实的性能数字 |
...
```mermaid
flowchart LR
  U[用户或客户端] --> UI[Floem UI 与交互层]
  UI --> C[状态与领域核心 待核验]
  C --> WASM[WASI 插件运行时]
  C --> LSP[LSP 客户端]
  LSP --> EXT[外部语言服务器 复用 VS Code 生态]
  C --> SSH[远程 SSH 编辑能力]
  C --> VIM[内置 Vim 模式]
  C --> OBS[风险与局限 功能成熟度 插件冷启动 团队规模]
```
