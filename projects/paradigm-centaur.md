...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 中心是一道"入口/身份 → 编排运行时 → 模型/工具/会话"的协作平台，对外接团队成员与外部系统，对内承担凭据隔离与自托管部署 | 边界为档案中"multiplayer / self-hosted / secure-agents / team-agents"标签与定位归纳，未见具体协议、数据库与部署单元 |
| 主路径 | 成员经入口发起任务 → 运行时调度模型与工具调用 → 凭据被代理使用但不向成员暴露 → 会话/状态/审计持续化可恢复 | "凭据安全边界、持久工作流、团队可见历史"为档案描述；模型协议、工具协议、持久化介质均无源码级证据 |
| 关键权衡 | 在"团队共享 + 自托管 + 凭据隔离"前提下，权衡扩展能力与权限模型、可观测性、供应商耦合；NOASSERTION 许可又叠加法务/采用风险 | 权衡源自档案定位与"个人 Agent → 团队 Agent"的威胁模型论述；凭据隔离的实现细节、License 走向均未公开 |
| 最小 PoC | 自托管部署、单一团队成员、单一模型供应商、有限工具集合与强审计，验证"Agent 用凭据而成员不可见"是否成立；验收含 License、安全、成本、可退出性 | 上述 PoC 步骤为档案支持的合理外推；具体部署形态、凭据隔离实现、模型/工具接入方式均需读源码或文档核验 |
...
```mermaid
flowchart LR
    U[团队成员或上游系统] --> I[入口与身份边界<br/>多用户接入]
    I --> C[项目核心<br/>Centaur 编排与运行时]
    C --> M[模型或推理服务<br/>供应商待核验]
    C --> T[工具与外部系统<br/>凭据被调用但对成员不可见]
    C --> S[会话 状态 审计<br/>持久工作流]
    C --> H[自托管部署单元<br/>centaur.run 形态待核验]
    L[License 风险边界<br/>NOASSERTION] -.约束.-> C
    待核验[具体协议 持久化 鉴权实现<br/>均需源码或文档验证] -.标注.-> C
```
