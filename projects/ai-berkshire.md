...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 编排层覆盖 Claude Code 与 Codex 两类 Coding Agent 入口，依赖 Python 与外部 LLM；不内置交易/经纪执行通道 | 来自分类、tags（claude-code, codex, llm）、language=Python；具体数据源/合规边界未在档案中给出 |
| 主路径 | 目标公司 → 四大师 Agent 并行分析 → 综合研究 → 输出研究/决策参考（无交易回写） | 仅源自档案"四大师方法论并行研究"与定位为"研究辅助而非交易系统"；执行层与持久化细节须以源码核验 |
| 关键权衡 | 多视角对抗分析带来的洞察增益 vs. 方法论编码本身的回测偏差、幸存者偏差与监管不确定性 | 风险条目（方法论≠能力、回测偏差、AI 投顾牌照）来自档案定性陈述，无回测或合规实证 |
| 最小 PoC | 单一公司、单一 Agent 视角（如巴菲特视角）、公开信息源、可审计日志，验证输出一致性与可解释性 | 采用建议引自档案"先在单一渠道、最小工具权限和可审计日志下验证"；性能/成本/SLO 数据档案未给出 |
...
flowchart LR
  T[目标公司]
  T --> A1[巴菲特 Agent<br/>护城河/管理层/估值]
  T --> A2[芒格 Agent<br/>多元思维模型]
  T --> A3[段永平 Agent<br/>本分/简单]
  T --> A4[李录 Agent<br/>中国视角/长期价值]
  A1 --> S[综合研究]
  A2 --> S
  A3 --> S
  A4 --> S
  S --> R[投资决策参考<br/>不含交易执行]
  C[外部边界: 监管/牌照<br/>待核验] -.约束.-> R
  X[风险边界: 回测偏差<br/>方法论≠能力] -.约束.-> S
```
