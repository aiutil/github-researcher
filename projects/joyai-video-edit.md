...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 京东官方背书的实时流式视频编辑系统，入口含在线 demo（joyai-labs.jd.com/v2v/）与 HuggingFace checkpoint，主干为 16B 参数 MMDiT + 因果视频 VAE + MLLM 条件编码器，需企业级 GPU 部署 | 16B 规模、消费级 GPU 支持在 TODO；硬件清单未披露 |
| 主路径 | 用户/摄像头流 → 因果 VAE 逐帧编码 → 16B MMDiT 自回归扩散编辑（带 MLLM 文本条件）→ 有界 KV-state 推理 + 对齐自回归分布匹配蒸馏 → 解码输出；arXiv 2608.03974 支撑 30.19 FPS @ 720p | 30.19 FPS 与四类加速为论文/README 自述，未独立复现；TTFT 未披露 |
| 关键权衡 | 因果逐帧（不等待未来帧、放弃全局优化）换实时性；KV-state 有界化换长序列内存可控；模型能力 vs 部署门槛（16B） | 流式因果带来的画质/一致性上限未有公开基准；端到端延迟与吞吐未区分 |
| 最小 PoC | 在具备企业级 GPU 的环境部署 HuggingFace checkpoint，跑通单条自然语言指令的流式逐帧编辑，复核 720p 30 FPS 与 TTFT；同步检查 Apache-2.0 条款与可审计日志方案；RTX 5090 支持列为后续验收 | 消费级 GPU（RTX 5090）支持明确在 TODO，不能作为本次 PoC 前提 |
...
```mermaid
flowchart LR
    U[用户或摄像头流] --> I[入口与身份边界 在线Demo与Checkpoint]
    I --> C[编排与运行时 Autoregressive-Diffusion 调度器]
    C --> CV[因果视频 VAE 逐帧编码 无未来帧]
    C --> MM[16B MMDiT 主干 待核验部署形态]
    C --> ML[MLLM 条件编码器 文本指令]
    CV --> MM
    ML --> MM
    MM --> K[有界 KV-state 推理 长序列内存控制]
    MM --> D[对齐自回归分布匹配蒸馏 加速]
    K --> O[解码输出 30.19 FPS 720p 论文声明未独立复现]
    D --> O
    C --> S[会话 状态 审计 实时性风险边界 TTFT未披露]
    O --> S
```
