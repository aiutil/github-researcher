...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | Slate 处于 React 富文本编辑的基础设施层，边界止于浏览器端编辑器框架；不内置后端协议、存储、协同传输或服务端渲染策略 | 档案仅描述其为 React 编辑器框架，未列出后端组件、部署形态或持久化方式 |
| 主路径 | 用户输入 → React 组件渲染层 → 基于 JSON 的文档模型与 transforms 命令式转换 → 插件机制覆盖渲染、快捷键与序列化 | 路径来自档案明确提到的 JSON 节点树、transforms API、插件架构、React 渲染同步 |
| 关键权衡 | 灵活性（任意自定义节点/渲染/快捷键）与 API 稳定性、学习成本之间的权衡——长期 Beta 状态、需自研协同与移动端支持是这一权衡的代价 | 权衡依据限于档案中"长期 Beta""学习曲线陡峭""协同需额外集成 Yjs""移动端支持有限"等已陈述事实 |
| 最小 PoC | 在 React 应用中接入 Slate 编辑器，验证自定义块节点渲染、键盘快捷键扩展、撤销重做与 HTML/Markdown 序列化四条最小路径 | 仅基于档案列出的插件覆盖范围（渲染、快捷键、序列化）与 transforms 支持撤销重做；具体序列化格式、性能基线未在档案中给出 |
...
```mermaid
flowchart LR
    U[用户交互 输入与选区] --> V[React 组件渲染层 基于 Slate]
    V --> M[JSON 文档模型 嵌套节点树]
    M --> T[Transforms 命令式转换 支持撤销重做]
    T --> P[插件机制 渲染 快捷键 序列化]
    P --> S[序列化输出 HTML 或 Markdown 待核验]
    Y[外部协同能力 Yjs 集成] -.可选扩展.-> M
    R[风险边界 长期 Beta 移动端有限 协同非原生] -.约束.-> V
```
