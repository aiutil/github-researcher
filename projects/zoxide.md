...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 本地 CLI 工具 + shell hook，单一 Rust 二进制，无服务端、无模型、无远端依赖；外部边界仅与 shell（bash/zsh/fish/nushell/elvish/PowerShell/xonsh）和 fzf 交互 | 边界基于档案明示的语言/标签/shell 支持清单；具体 hook 注入点与数据落盘格式需源码核验 |
| 主路径 | shell `cd` → zoxide hook 记录访问 → 写入本地 frecency 数据库 → 用户键入 `z <关键字>` → 查询+排序 → `cd` 到目标目录（多匹配时调用 fzf 交互选择） | 主路径以档案"frecency + fzf 集成 + cd 别名"为据；排序权重与持久化格式未给出具体实现，待核验 |
| 关键权衡 | 启动延迟与功能丰富度的取舍（Rust 单二进制选前者）；shell 兼容性广度 vs 单一 shell 深度优化；学习用户习惯的正确率 vs 偶发误猜时的交互兜底（fzf） | 权衡来自档案"启动微秒级 / 7+ shell 覆盖 / frecency 不总是猜对"的描述；性能数字未经基准复测 |
| 最小 PoC | 在单一 shell（如 bash）安装二进制 → 跑一周日常目录访问 → 验证 `z <关键字>` 命中率与启动感知延迟 → 确认数据库可读/可备份 → 再扩展至 zsh、PowerShell 等 | PoC 假设基于"安装即用 + 可移植数据"的档案描述；具体安装命令、数据库路径、fzf 依赖前提需以 README/源码核验 |
...
```mermaid
flowchart LR
    U[终端用户] --> SH[Shell 钩子 hook<br/>bash/zsh/fish/nushell/elvish/PowerShell/xonsh]
    SH -->|记录 cd 事件| Z[zoxide Rust 单二进制]
    Z --> DB[(本地 frecency 数据库<br/>路径+分数 格式待核验)]
    Z --> FZF[fzf 交互选择<br/>外部依赖]
    FZF -->|选中目标| Z
    Z -->|执行 cd| SH
    SH --> U
```
