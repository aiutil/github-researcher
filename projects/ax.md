...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 位于入口/身份层与模型、工具/外部系统之间的"Agent 运行时"层，承载持久化、审计与策略执行，独立于 Agent 框架 | 仅依据项目分类、标签（agent-runtime, distributed, kubernetes）与档案定位描述；具体接口协议、持久化后端、K8s CRD/Operator 形态未在档案中证实 |
| 主路径 | 上游触发 → 入口与身份 → 编排与运行时 → 调用模型与工具 → 会话/状态/审计回写 | 依据档案"关键技术亮点"四项能力与"架构启发"中"运行时层"抽象；具体控制流、API、序列化格式均为待核验 |
| 关键权衡 | 把"智能编排"与"运行治理"解耦带来的框架无关收益，对比 Go/Python 跨语言生态集成的现实成本与 Temporal/Restate 已成熟方案的竞争压力 | 基于档案"与同类项目的关系"与"风险/局限"；缺乏 ax 自身基准、采用案例与维护活跃度的硬数据 |
| 最小 PoC | 单入口渠道 + 最小工具白名单 + 全量审计日志接入，验证崩溃恢复断点、策略拦截、分布式调度三项核心能力，再决定扩展接入面 | 档案未给出官方示例与文档指引；PoC 范围需以 ax 源码/官方文档二次核验，agentexecutor.io 是否提供接入规范属待核验 |
