...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | LMCache 是面向 LLM 推理服务的 KV Cache 解耦层，介于推理引擎（明确提及 vLLM/PagedAttention）与上层请求之间，对外提供跨请求前缀缓存复用能力。 | 档案明确其为"LLM 推理的 KV Cache 加速层"，与 vLLM 兼容；是否同时支持 SGLang、TensorRT-LLM 等其他引擎档案列为"待核验"。 |
| 主路径 | 请求进入后先做前缀匹配 → 命中则复用 KV Cache 跳到推理；未命中则计算 KV Cache 并按 GPU→CPU→分布式层级写入 LMCache，再执行推理并回写元数据。 | 路径节点来自档案"关键技术亮点"与架构启发流程图；具体命中判定算法、存储后端选型、序列化协议未在档案中给出。 |
| 关键权衡 | 跨请求复用收益 vs vLLM 内置 prefix caching 的潜在功能重叠，以及缓存命中率/失效管理复杂度 vs GPU 显存节省与延迟下降。 | 档案明确列出 vLLM 强依赖、缓存失效复杂性、收益场景有限三项风险；未提供具体命中率、显存收益数字或对比 benchmark。 |
| 最小 PoC | 选定单一 vLLM 推理入口，固定 system prompt + few-shot 模板构造可复用前缀，验证跨请求 KV Cache 命中率与首 token 延迟变化，确认与 PagedAttention 兼容无回归。 | 集成对象（vLLM）已明确，验证指标（命中率、延迟）来自档案目标描述；具体部署形态、存储后端、版本要求"待核验"。 |
...
```mermaid
flowchart LR
    A[LLM 推理请求] --> B{前缀 KV Cache 命中?}
    B -- 命中 --> C[复用已缓存 KV]
    B -- 未命中 --> D[计算 KV Cache]
    D --> E[LMCache 多层级存储<br/>GPU 内存 → CPU 内存 → 分布式存储<br/>后端选型待核验]
    E --> F[vLLM / PagedAttention<br/>执行注意力与推理]
    C --> F
    F --> G[返回推理结果]
    G --> H[更新缓存元数据<br/>命中率与失效策略待核验]
    F -. vLLM 内置 prefix caching 风险边界 .-> I[vLLM 上游演进<br/>功能重叠待核验]
    H -. 外部存储后端边界 .-> J[分布式存储 / 对象存储<br/>协议待核验]
```
