...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | Oil Motion 是 Skill 形态的编排层：Agent 安装入口 → 关键画面确认 → AI 视频生成 → 逐帧质量过滤与按显示尺寸压缩 → 前端交互绑定（滚动/鼠标/拖动/触摸/设备方向） | 档案未给出具体模型供应商、协议、SDK；压缩算法与帧质量判定细节未披露 |
| 主路径 | 用户描述交互动画意图 → Agent 编排三段式工作流（关键画面 → AI 视频填充中间帧 → 帧检查+压缩+绑定）→ 产出可由前端输入驱动进度的帧序列资源 | "三段式工作流"与"进度映射引擎"来自 README 转述；运行时实现、模型调用细节未证实 |
| 关键权衡 | 在 AI 视频生成成本、帧序列体积（移动端加载性能）、帧数与交互平滑度的三角约束下，Skill 用"按显示尺寸压缩+逐帧过滤"换取跨输入源的统一映射机制 | 档案未给出体积基准、帧数推荐、生成时延与失败回退策略 |
| 最小 PoC | 单渠道（仅滚动驱动）、固定帧数与显示尺寸、单一 AI 视频生成后端，先验证"输入进度 → 帧索引 → 资源加载"的最小闭环与体积/性能基线 | API key 配、最小可运行示例、CI/部署形态未在档案中给出，需以源码核验 |
...
```mermaid
flowchart LR
    U[使用者或上游Agent] --> I[入口与Skill安装边界]
    I --> C[项目编排与三段式工作流<br/>关键画面确认/帧生成/帧检查与压缩]
    C --> M[AI视频生成服务<br/>模型供应商待核验]
    C --> T[前端交互绑定<br/>滚动/鼠标/拖动/触摸/设备方向]
    C --> S[帧序列资源状态<br/>按显示尺寸压缩 持久化方式待核验]
    M --> C
    T --> C
    S -.待核验.-> T
</mmd>
