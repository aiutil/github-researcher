...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 定位为 Agent 与上游调用方、模型服务、工具/数据源之间的安全运行时层，由 NVIDIA 以 Rust 实现，承担四层策略隔离与会话级沙箱 | 仅基于档案描述的分类、标签与定位；具体入口协议、模型适配层与外部工具接入清单未在档案中给出，待核验 |
| 主路径 | Agent 会话进入运行时 → 经 Filesystem/Network/Process/Inference 四层 YAML 策略校验 → 在隔离会话中执行工具与推理调用 → 状态/审计回写；GPU 直通为可选推理路径 | 路径由档案中“四层策略隔离 + YAML 热加载 + 会话级隔离 + GPU 直通”抽象得出，私有协议与持久化实现细节未披露 |
| 关键权衡 | 会话级隔离强度（Rust + GPU 直通） vs 跨硬件可移植性与生态锁定风险；热加载 YAML 策略带来的运维敏捷性 vs 默认最小权限的策略维护成本 | 权衡来自档案明确列出的“NVIDIA 锁定风险”“非 NVIDIA 硬件支持不足”等局限；未给出量化对比 |
| 最小 PoC | 单一 Agent 渠道、最小工具白名单、开启审计日志的会话，部署于档案所述的 Helm K8s 或 OpenShift 形态上，验证 GPU 直通推理路径与策略热加载生效 | PoC 形态仅引用档案“新增 Helm K8s 部署 + OpenShift 支持 + GPU 直通”与“最小权限”原则；具体镜像、Chart 与 GPU 设备插件配置未在档案中提供，待核验 |
...
```mermaid
flowchart LR
    A[Agent 或上游编排器] --> B[OpenShell 入口与会话边界]
    B --> C{四层策略校验 YAML 热加载}
    C -->|放行| D[Rust 沙箱 会话级隔离]
    C -->|拒绝| E[审计 拒绝事件]
    D --> F[Filesystem 隔离]
    D --> G[Network 隔离]
    D --> H[Process 隔离]
    D --> I[Inference 隔离 含 GPU 直通 待核验]
    D --> J[工具与外部系统调用]
    D --> K[会话状态 审计日志]
    F --> K
    G --> K
