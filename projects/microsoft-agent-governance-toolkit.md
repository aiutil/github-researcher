...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | 治理工具包位于 Agent 编排层，覆盖身份/入口、运行时、模型、工具、会话/审计之间的边界声明与执行 | 组件名称（Policy Engine、Zero-Trust Identity、Execution Sandbox、Reliability Engineering、OWASP Agentic Top 10 映射）来自档案；具体 SDK、传输协议与持久化方案未在档案中证实 |
| 主路径 | 操作请求 → 身份与策略校验（零信任 + 声明式策略）→ 在执行沙盒内调用模型/工具 → 会话/审计回写 → 异常检测闭环 | 主路径结构由档案"架构师速览/主路径"行抽象而成；运行时实现、调用栈、状态机细节均待源码核验 |
| 关键权衡 | 自主性 vs 治理强度、性能开销 vs 可审计性、与特定 Agent 框架的耦合 vs 通用覆盖面 | 权衡来自档案"风险/局限"段对"性能开销""集成度待验证""标准竞争"的归纳；性能数字、延迟基准未给出 |
| 最小 PoC | 单入口渠道 + 最小工具权限 + 声明式策略 + 隔离执行沙盒 + 可审计日志，覆盖 OWASP Agentic Top 10 全 10 项映射 | PoC 范围依据档案"架构启发"与"采用建议"行；具体策略语言、沙盒后端（容器/VM/WASM）以及审计存储未在档案中说明 |
...
```mermaid
flowchart LR
    U[使用者或上游系统] --> I[入口与零信任身份边界]
    I --> PE[策略引擎 声明式策略 审计待核验]
    I --> SB[执行沙盒 隔离运行时 待核验]
    PE --> C[编排与可靠性工程 异常检测]
    SB --> C
    C --> M[模型或推理服务 外部边界]
