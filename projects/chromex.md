...
| 决策问题 | 研究判断 | 证据边界 |
|---|---|---|
| 系统边界 | Chromex 是 Chrome Side-Panel Extension 形态的 Browser Agent，编排层位于扩展内，对外向 Codex 引擎、当前页面 DOM/文本、语音/图片输入和标签页状态开放 | 仅据档案标签与定位描述，未见源码层级的组件清单 |
| 主路径 | 用户在 Side-Panel 发起请求 → 扩展运行时聚合页面上下文与多模态输入 → 调用 Codex 推理 → 回写会话与标签页状态 | "Codex 驱动、页面上下文、多模态、标签页管理"为档案原文，协议、持久化与部署形态待核验 |
| 关键权衡 | 嵌入式 Side-Panel 路线（轻、与浏览器深度整合）换取对 Chrome Side-Panel API 与 Codex 的强耦合，平台策略与模型窗口构成天花板 | 档案明确点出平台依赖、Codex 绑定、上下文长度与 Google 原生竞品风险 |
| 最小 PoC | 单标签页场景：固定 Chrome 侧边栏 + 单一 Codex 调用 + 页面 DOM/文本上下文 + 语音/图片各一条路径 + 可审计日志，先验证再扩展多标签页与多模态 | 仅据档案建议与亮点反推，未含 PoC 性能/合规基准 |
...
flowchart LR
    ...
    C --> M[Codex 推理引擎]
    M --> C
    C --> S[会话 标签页状态 审计日志 待核验]
    S --> I
    C -.平台策略风险.-> R[Chrome Side-Panel API 边界 待核验]
```
