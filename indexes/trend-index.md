...
### anishfn/shapeshift（387 stars · fork 43）· SewCabinSpout/cleanupper（723 stars · fork 0）· miuuyy/Astra-Ares（234 stars · fork 15）· edison-land/paragravity（186 stars · fork 16）· lhlGitHub/threejs-architecture-effects（172 stars · fork 35）
...
| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|---:|---|---|---:|
| 1 | Jev 应用层 UI 端具身形态——anishfn/shapeshift 2 天 387⭐ ⑂43 fork/star 11.1%（TypeScript · MIT · 12.1 MB · 一个文本框根据输入自动变出对应卡片 · 一句话判断 14 个 typed questions 决定走哪张卡片 · 日期 / 金额 / 单位 / 数学计算全用确定性代码 · 内置离线关键词分类器兜底 · 18 类卡片 Event / Reminder / Checklist / Timer / Color / Split / Expense / Calculate 等 · 滞回状态机 challenger 赢两次才换 · URL 参数 ?debug=1 ?demo=1&loop=1 可视化 · Bun 1.2+ + Next.js + Three.js + Excalidraw · 与昨日 Rizzo-AI-Academy/rizzo-flow「Spark-X2.5 4B Jev 兼容 0 generated tokens」+ TianyuCodings/JevHarness「LLM 写 harness + GEPA 全轨迹反思」同构「Jev 应用层 + 内核分层」但推到「UI 端 18 类卡片具身 + 决定走哪张卡片 + 确定性代码计算 + 内置离线兜底 + 滞回状态机 + URL 参数可视化 + Next.js」具身交互形态 · 18 类卡片 · online demo shapeshiftui.vercel.app · README 明示「Jev decides, code computes」） | anishfn/shapeshift | 88 |
| 2 | Jev 在 Codex 任务里动态选 reasoning effort——miuuyy/Astra-Ares 2 天 234⭐ ⑂15（JavaScript · MIT · 716 KB · 单独安装打过补丁的 Codex CLI 不动原有 codex 与 Codex desktop · OpenRouter 默认 · `ares configure` 配 key · `/model` 选 Astra / Sol / Luna Ares · 每条选 underlying 模型固定 Jev 选 reasoning effort · 调 effort 用 GPT-6 原生机制不破坏前缀缓存 · 显示 Jev LOW → HIGH ✓ APPLIED · Step N · 321 ms · macOS Apple Silicon 已本地构建测试 · macOS Intel / Linux 路径提供 · Windows 不支持 Unix-socket 集成 · 与昨日 Rizzo-AI-Academy/rizzo-flow + anishfn/shapeshift 同构「Jev + Codex / 严肃应用层」但推到「Codex 任务动态推理 effort + 不破坏前缀缓存 + 显示应用状态 + OpenRouter 默认 + ares configure 配置 + 仅 macOS Apple Silicon 已 acceptance-test」严肃工程化形态） | miuuyy/Astra-Ares | 85 |
| 3 | macOS 终端清理 CLI 替代 CleanMyMac——SewCabinSpout/cleanupper 1 天 723⭐ ⑂0（JavaScript · MIT · 253 KB · macOS 11+ · Node.js 18+ · 一行 `xcode-select --install && mkdir diskclean && cd diskclean && npm install github:SewCabinSpout/cleanupper` 即装 · cleanupper scan 列出 user-caches / xcode-deriveddata / npm-cache / homebrew / browser-cache 五类回收空间 · Trash-first 删除可还原 · 保护路径黑名单让灾难性删除结构性不可能 · Xcode / npm / Yarn / pnpm / pip / uv / CocoaPods / Gradle / Cargo / Go caches 全覆盖 · purge 命令扫 stale node_modules / target / .venv across your projects · --json --yes 让 CI / cron 可脚本化 · Zero telemetry 全程不联网 · 与昨日 unreallabsai/unreal-agent「async-first harness 八组件」同构「严肃工程化个人开发者作品」但推到「macOS CLI + Trash-first 安全模型 + 保护路径黑名单 + Zero telemetry + JSON 输出 + CI 脚本化 + 多语言依赖清理」安全工程化形态 · fork/star 0% 极干净个人开发者信号 · 723⭐ / fork 0 高质量粉丝结构） | SewCabinSpout/cleanupper | 82 |
| 4 | Google Antigravity 多账号并行沙箱管理器——edison-land/paragravity 2 天 186⭐ ⑂16（Python · MIT · 136 KB · 一行 macOS `curl install.sh | bash` · Windows PowerShell clone · pgrav create work · 100% 非侵入基于 Chromium/Electron `--user-data-dir` · 零二进制补丁 · 0 MB idle overhead · 纯 Python 3 零外部依赖 · Native Google OAuth 完整登录流 · 自动生成 macOS .app 包 Spotlight 索引 · Windows .lnk 快捷方式 · SSH / config 三档 --links full / minimal / none · --inherit-config -i 继承宿主配置 · --no-mcp 排除 MCP · 多账号 side-by-side 独立窗口 · 每实例 extensions / local storage / indexedDB / 聊天历史 完全隔离 · 流程可启动时不重启 · 与昨日 09-22 jev-chat 系列 + 09-23 unreallabsai/unreal-agent 同构「严肃工程化个人开发者工具 + 隐私 / 安全边界」但推到「Chromium / Electron user-data-dir 多账号并行 + 完整 OAuth 流 + 自动 .app / .lnk 集成 + Python 3 零依赖 + 三档链接策略 + 继承 / 排除 MCP」跨平台形态） | edison-land/paragravity | 78 |
| 5 | Agent Skill 用 Three.js 动态组装古建——lhlGitHub/threejs-architecture-effects 2 天 172⭐ ⑂35（TypeScript · MIT · 66.3 MB · Codex / Claude Code / Cursor 三平台安装到 ~/.codex/skills / ~/.claude/skills / ~/.cursor/skills · Node.js 22.13+ WebGL2 浏览器 · `npm ci && npm run dev` · 一个 0-1 时间轴 play / pause / scrub backward · 程序化砖 / 木 / 抹灰 / 瓦 / 石 / 青铜 PBR · 飞檐细赏 + 石狮近观 镜头 · Vite + React + Three.js starter · 完全程序化几何无付费模型 · scaffold.mjs 拷 starter 不覆盖 · 中文 / 英文 README · 与 09-19 ~ 09-22 各 agent skill / harness 同构「Agent Skill 严肃工程化」但推到「Three.js 古建程序化动态组装 + 0-1 时间轴 + 三平台 skills 安装 + 程序化材质 + Vite + React starter + 中文 / 英文 README」3D 严肃工程化形态 · fork/star 20.3% 较高 Agent Skill 信号） | lhlGitHub/threejs-architecture-effects | 75 |

#### 当日重点项目

- 🎭 anishfn/shapeshift (2 天 387⭐ ⑂43) — 「一个文本框变出对应卡片」的 Jev 决策 UI；TypeScript · MIT · 12.1 MB · 一句话判断 14 个 typed questions 决定走哪张卡片 · 日期 / 金额 / 单位 / 数学计算全用确定性代码 · 内置离线关键词分类器兜底 · 18 类卡片 · 滞回状态机 challenger 赢两次才换 · URL 参数 ?debug=1 ?demo=1&loop=1 可视化 · Bun 1.2+ · Next.js + Three.js · fork/star 11.1%，Score 88
- ⚡ miuuyy/Astra-Ares (2 天 234⭐ ⑂15) — GPT-6 Codex 任务自适应推理 effort（Jev 选 effort）；JavaScript · MIT · 716 KB · 单独安装打过补丁的 Codex CLI · OpenRouter 默认 · `ares configure` 配 key · `/model` 选 Astra / Sol / Luna Ares · Jev 调 GPT-6 reasoning effort 不破坏前缀缓存 · 显示 Jev LOW → HIGH ✓ APPLIED · Step N · 321 ms · macOS Apple Silicon 已本地构建测试 · fork/star 6.4%，Score 85
- 🧹 SewCabinSpout/cleanupper (1 天 723⭐ ⑂0) — macOS 终端清理 CLI 替代 CleanMyMac；JavaScript · MIT · 253 KB · macOS 11+ · Node.js 18+ · 一行 `xcode-select --install && mkdir diskclean && cd diskclean && npm install github:SewCabinSpout/cleanupper` 即装 · cleanupper scan 列出 user-caches / xcode-deriveddata / npm-cache / homebrew / browser-cache 五类回收空间 · Trash-first 删除可还原 · 保护路径黑名单让灾难性删除结构性不可能 · Xcode / npm / Yarn / pnpm / pip / uv / CocoaPods / Gradle / Cargo / Go caches 全覆盖 · purge 扫 stale node_modules / target / .venv · --json --yes CI 脚本化 · Zero telemetry 全程不联网 · fork/star 0%，Score 82
- 🌌 edison-land/paragravity (2 天 186⭐ ⑂16) — Google Antigravity 多账号并行沙箱管理器；Python · MIT · 136 KB · 一行 macOS `curl install.sh | bash` · Windows PowerShell clone · pgrav create work · 100% 非侵入基于 Chromium/Electron `--user-data-dir` · 零二进制补丁 · 0 MB idle overhead · 纯 Python 3 零外部依赖 · Native Google OAuth 完整登录流 · 自动生成 macOS .app 包 Spotlight 索引 · Windows .lnk 快捷方式 · SSH / config 三档 --links full / minimal / none · --inherit-config -i 继承宿主配置 · --no-mcp 排除 MCP · fork/star 8.6%，Score 78
- 🏯 lhlGitHub/threejs-architecture-effects (2 天 172⭐ ⑂35) — Agent Skill 用 Three.js 动态组装古建；TypeScript · MIT · 66.3 MB · Codex / Claude Code / Cursor 三平台安装到 ~/.codex/skills / ~/.claude/skills / ~/.cursor/skills · Node.js 22.13+ WebGL2 浏览器 · 一个 0-1 时间轴 play / pause / scrub backward · 程序化砖 / 木 / 抹灰 / 瓦 / 石 / 青铜 PBR · 飞檐细赏 + 石狮近观 镜头 · Vite + React + Three.js starter · 完全程序化几何无付费模型 · scaffold.mjs 拷 starter 不覆盖 · 中文 / 英文 README · fork/star 20.3%，Score 75



## 2026-09-23

### jev-chat/jev-chat-jarvis（3,660 stars · fork 821）· unreallabsai/unreal-agent（746 stars · fork 35）· jev-chat/jev-chat-windows（249 stars · fork 56）· freestylefly/WeChatBridge（238 stars · fork 133）· deepopen-com/deepopen（206 stars · fork 1）

> 证据边界：项目名称、星标、tags、description、license、size、forks 来自 2026-09-23 GitHub Search API 公开元数据（created_at / pushed_at / stargazers_count / forks_count / language / description / topics / license / size）+ 各项目 README 公开摘录（API readme 字段 base64 解码）。"X 天 Y⭐"基于 created_at→2026-09-23 总星数除以经过天数（粗略下限估计），stargazers REST endpoint 需要认证故未做精确单日增量统计。**今日观察窗为 2026-09-21 创建且截至 2026-09-23 仍在快速增长的 5 个非游戏类高质量工程型项目**；09-23 当日新创仓库在 GitHub API 暴露窗口 < 12 小时，本批重点放在 09-21 ~ 09-22 创建且仍持续增长的项目。**今日最大事件是 `jev-chat/jev-chat-jarvis` 2 天 3660⭐ / fork 821 / fork/star 22.4% — 昨日 Finderchangchang/jev-chat-JARVIS（833⭐ / fork 361 / fork/star 43.3% 个人开发者形态）的官方 org fork 多端展开**——把昨日「Android 端无障碍读节点 → Jev 一次 7 道题判断 → 生成模型起草 3 候选 → Jev 排序 → 半透明悬浮窗 → 复制 / 填入」整条链从个人开发者形态推到官方 org fork 形态：APK 签名 v1.3 · 网站 chatjevs.com · 三路 API key 拆分（判断 / 回复 / 视觉）· 用户本地知识库与联系人档案 · 9 个原生入口（微信 8.0.78 + QQ 9.3.50 + X 12.25 + 飞书 1.3 起 ML Kit 离线 OCR + 任意 App 悬浮窗菜单「截屏识别一次」）· 桌面端 / 网页 ⏳ 规划 + 同步发布 macOS 版（jev-chat-mac）+ Windows 版（jev-chat-windows 2 天 249⭐）+ ⏳ 桌面网页；3660⭐ / 821 fork 是 09-23 当日 GitHub Search created 2026-09-21..2026-09-23 stars>200 全站 trending 第一位（远超 unreallabsai/unreal-agent 746⭐ / freestylefly/WeChatBridge 238⭐ / deepopen-com/deepopen 206⭐ / jev-chat/jev-chat-windows 249⭐）。**今日 5 个项目分三条主线**：**A. Jev / 聊天侧 AI 副驾生态组织化与多端展开**（jev-chat org fork + Windows 端口 + macOS 微信 Share Extension + 多 Agent 入口）——把昨日 Finderchangchang/jev-chat-JARVIS「Android 个人开发者形态」推到「官方 org fork + macOS + Windows + 多 App 入口」组织化形态；**B. Jev / System One 决策模型严肃工程化与挑战者**（unreal-agent Go async-first harness + deepopen Laya 改进三检查点）——把昨日 agent SDK / harness fork 范式同构「Agent harness 严肃工程化」推到「async-first + Session 可 fork + Tool translator 严格同步不 I/O + Operation 版本化」具体路径 + 「非自回归 System 1 决策引擎 Laya 改进 + ModernBERT-large / mmBERT-base + 32.8 ms + 7.8× Jev 速度 + Apache-2.0 + pip install」严肃工程化形态；**C. 跨平台聊天侧 AI 副驾 / Agent 转发 / 知识库沉淀**（WeChatBridge macOS 微信 Share Extension + jev-chat-windows WGC 截图 + RapidOCR）——把昨日 Jev 副驾 + 决策模型生态推到「macOS 微信原生 Share Extension + Developer ID + 公证 + 双语界面 + 多 App 入口」+「Windows WGC 截图 + RapidOCR + exe 内置 + 注册表存 key + DeepSeek 国内直连」+「Obsidian 沉淀 + SKILL.md 兼容 Agent + 场景化提示词」三个跨平台分支。**与昨日「个人开发者形态严肃工程化 + Jev 去 SaaS 化 + 跨 CLI 检索」三主线不同**，今日主线转向「组织化官方 fork + 多端展开 + Sponsor + 网站」+「async-first harness + Apache-2.0 非自回归决策」+「macOS native Share Extension + 9 入口分发 + Developer ID + 公证」三个严肃工程化 + 多端展开的演化形态。

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|---:|---|---|---:|
| 1 | jev-chat-jarvis 组织化（个人 fork → 官方 org fork）——jev-chat/jev-chat-jarvis 2 天 3660⭐ ⑂821 fork/star 22.4%（Kotlin · MIT · 23.49 MB · 微信 8.0.52+ 混淆节点 8.0.78 实测可读 · QQ 9.3.50 群聊 · X 12.25 中文 · 飞书 1.3 起 ML Kit 离线 OCR 兜底 · 三路 API key 拆分：判断 / 回复 / 视觉 · 用户本地知识库与联系人档案 · APK 签名 v1.3 · 网站 chatjevs.com · 9 个原生入口 · 任意其它 App 用悬浮窗菜单「截屏识别一次」整屏 OCR · 桌面端 / 网页 ⏳ 规划 · 同步 macOS + Windows · 与昨日 Finderchangchang/jev-chat-JARVIS 833⭐ / fork 361 / fork/star 43.3% 个人开发者形态形成「个人 fork → 官方 org fork → 多端展开」演化 · README 明示「只读你自己设备上你自己有权查看的聊天」+「程序只把回复填进输入框从不自动发送不碰转账红包收款」） | jev-chat/jev-chat-jarvis | 92 |
| 2 | jev-chat-windows 同内核跨平台到 Windows——jev-chat/jev-chat-windows 2 天 249⭐ ⑂56 fork/star 22.5%（Python · PyQt · MIT-like · 微信 Windows 4.x · WGC 窗口截图 + RapidOCR 离线 OCR · exe 内置 146 MB · 注册表 HKCU\Environment 存 API key · 起草默认 DeepSeek 官网国内直连 · 判断 OpenRouter / TypeSafe 直连二选一 · 群聊指定回复对象 · 调试视图画框 · 1623 KB · 与昨日 Finderchangchang/jev-chat-JARVIS 同构「Jev + 微信 + 用户最终 gate」但推到「Windows 微信 + WGC 截图 + RapidOCR 离线 + exe 内置 + 注册表存 key + DeepSeek 国内直连」具体路径 · README 明示「只读自己电脑上自己本来就有权查看的对话」+「只截自己的微信窗口 + 本地离线 OCR RapidOCR」+「不 hook、不注入、不读微信数据库、不解密、不碰微信进程内存」+「截图只在内存里 · 全程不写磁盘、不进日志、不上传」） | jev-chat/jev-chat-windows | 85 |
| 3 | WeChatBridge macOS 微信合并转发 Share Extension → 9 入口——freestylefly/WeChatBridge 2 天 238⭐ ⑂133 fork/star 55.9%（Swift 6.0 · MIT · macOS 14+ · Developer ID 签名 + Apple 公证 0.1.14 DMG · 微信 4.1.13+ 多选聊天记录「合并转发」菜单 → Share Extension → Codex / Claude / 豆包 / 千问办公 / WorkBuddy / WeSight / Obsidian / 剪贴板 / 自定义 九入口 · 场景化提示词 + 技能中心 + SKILL.md 兼容 Agent · 双语界面 · 26298 KB · 与昨日 09-21 ~ 09-22 各 Jev 副驾同构「微信聊天侧 AI 副驾 + 用户最终 gate」但推到「macOS 微信原生 Share Extension + Developer ID + 公证 + 双语界面 + 多 App 入口 + Obsidian 沉淀 + 技能中心」严肃工程化 · 与 jev-chat 系列同构「聊天 → Agent」但 jev-chat 是「读屏 → Jev 判断 → 填入」，WeChatBridge 是「多选转发 → 多 Agent → 用户手动激活」） | freestylefly/WeChatBridge | 82 |
| 4 | unreal-agent async-first Go Agent harness——unreallabsai/unreal-agent 2 天 746⭐ ⑂35 fork/star 4.7%（Go · MIT · Session inbox / Coordinator / Session store / Context builder / LLM Adapter / Tool registry / Operation manager 八组件 · 8 项术语严格定义 · 3 项不变量 · Session 可 fork · Operation 版本化 · 组件可替换 · 远程 sandbox proxy operations · Bash / ViewImage / skill-use 固定工具 · 1061 KB · 与 09-19 ~ 09-22 各 agent SDK / harness 同构「Agent harness 严肃工程化」但推到「async-first + Session 可 fork + Tool translator 严格同步不 I/O + Operation 版本化 + 组件可替换 + 远程 sandbox proxy operations manager」具体路径 · OpenAI 兼容 LLM Adapter） | unreallabsai/unreal-agent | 80 |
| 5 | DeepOpen 非自回归 System 1 决策引擎 Laya 改进三检查点——deepopen-com/deepopen 2 天 206⭐ ⑂1 fork/star 0.5%（Python · Apache-2.0 · ModernBERT-large 421M 英文 39.5 ms / mmBERT-base 322M 多语言 1024 ctx / ModernBERT-large 421M 类型决策 1024 ctx · 单请求 32.8 ms · 10 题批量 72.3 ms · T4 单卡 103-332 问题/秒 · 7.8 倍 Jev 速度 · 2000 样本基准 0.766 准确率超 Jev 1.13.0 0.727 · Brier 0.062 · ECE 0.081 · Apache-2.0 完全开源权重 · pip install deepopen · Router preload · CLINC150 / Banking77 榜单第 2 / 第 5 · 2652 KB · 与昨日 Heman10x-NGU/openJev-verdict-2.0「Non-Autoregressive Decision Engine 击败 Jev & Laya」+ Rizzo-AI-Academy/rizzo-flow「Spark-X2.5 4B Jev 兼容 0 generated tokens」同构「Jev / Laya 决策模型挑战者 + 开源权重 + 非自回归 + Apache-2.0」但推到「三检查点 + Laya 改进 + 智能路由器 + 32.8 ms + 7.8× Jev 速度 + Apache-2.0 + pip install + ECE 0.081 + 2000 样本 0.766 准确率 + 多语言 1024 ctx + 高棉语 0 → 0.451 准确率」严肃工程化） | deepopen-com/deepopen | 78 |
