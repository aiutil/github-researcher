...
### Haleclipse/CometixCode（325 stars · fork 17）· rmalde/minecraft-agent（259 stars · fork 17）· wuyoscar/jev-skill（142 stars · fork 3）· ghuntley/underclass（107 stars · fork 5）

> 证据边界：项目名称、星标、tags、description、license、size、forks 来自 2026-09-21 GitHub Search API 公开元数据（created_at / pushed_at / stargazers_count / forks_count / language / description / topics / license / size）+ 各项目 README 公开摘录（API readme 字段 base64 解码）。"X 天 Y⭐"基于 created_at→2026-09-21 总星数除以经过天数（粗略下限估计），stargazers REST endpoint 需要认证故未做精确单日增量统计。**今日观察窗为 2026-09-20 创建且截至 2026-09-21 仍在快速增长的 4 个非游戏类高质量工程型项目**；09-21 当日新创仓库在 GitHub API 暴露窗口 < 12 小时，本批重点放在 09-20 创建且仍持续增长的项目。**今日最大事件是 `Haleclipse/CometixCode` 1 天 325⭐ / fork 17 / fork/star 5.2% — Anthropic Claude Code TUI 的 Rust 1:1 重实现**——把 Anthropic 官方 Claude Code 的 React+Ink terminal UI 用 Rust 重写到 iocraft retained-mode TUI framework 上，每文件按 1:1 映射（TypeScript 组件→Rust 组件、hook→hook、deliberate deviation 记录在源码里）；自带 CometixTUI fork（row-level diffing + SIGCONT self-healing + IME cursor + bracketed paste + grid layout）；interactive loop + tool execution + permissions + MCP + slash commands 已实现；Rust 1.88+ edition 2024 最低依赖，无系统库、无 pkg-config；AGPL-3.0 严肃许可 + README 明示「Unofficial project, not affiliated with Anthropic」+「Claude/Claude Code are trademarks of Anthropic, PBC」+「Nothing here is endorsed by or supported by Anthropic」——是「Claude Code 周边生态 + Rust 重实现 + 严肃许可 + 独立开发者不冒认」的工程化形式；5919 KB + 1 天 325⭐ 是 09-21 当日全站 trending 第二位（仅次于 mizorewww/laya-mlx 09-19 创建的 1519⭐ 持续填充）。**今日 4 个项目分三条主线**：**A. Claude Code 生态 TUI 重实现 + 严肃许可**（CometixCode Rust 重写 + AGPL-3.0）——把昨日 09-19 ~ 09-20 各 agent SDK / harness fork 范式同构「Claude Code 替代 / 复用 / 重实现」但推到「terminal UI 1:1 重实现 + Rust + AGPL-3.0」领域；**B. Jev 决策模型具身应用 + 可执行技能合集**（minecraft-agent 端到端通关 + jev-skill 90 场景 9 技能）——把 09-15 ~ 09-20 五日 Jev 决策模型从「API + SDK + 资源聚合」推到「具身智能 agent 动作选择 + 可执行场景 + 可安装技能」领域；**C. AI Coding 多账户 / 多订阅池化代理**（underclass sticky session + 冷却 + fail-fast）——把昨日 09-17 clawback/claude-code-cost-ledger「session 层成本治理 + canonical JSON + buckets.yaml」同构「AI Coding 多账户 / 多订阅治理」但推到「OpenAI 兼容代理 + sticky session + 冷却 + fail-fast」运行时领域。**与昨日「网络主权 + Jev 生态去 SaaS + 中文场景本地工具」三主线不同**，今日主线转向「Claude Code 周边生态 + Rust 重写 + AGPL-3.0 严肃许可」+「Jev 决策模型具身应用与可执行技能合集」+「AI Coding 多订阅池化代理」三主线——Jev 生态从 09-18 的「单日爆发 + 09-19 持续填充 + 09-20 去 SaaS 化 + 资源聚合」推到 09-21 的「具身应用 + 可执行技能合集 + 多模型生态」（minecraft-agent 端到端通关 + jev-skill 90 场景）；AI Coding 治理从「session 层成本 + 本地优先 + 决策模型自托管」推到「多订阅池化代理 + sticky session + 冷却 + fail-fast」运行时领域。

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|---:|---|---|---:|
| 1 | claude-code-tui-rust-port / Anthropic Claude Code TUI 的 Rust 1:1 重实现——Haleclipse/CometixCode 1 天 325⭐ ⑂17 fork/star 5.2%（Rust 2024 edition · AGPL-3.0 · iocraft retained-mode TUI framework · 自带 CometixTUI fork row-level diffing + SIGCONT self-healing + IME cursor + bracketed paste + grid layout · TypeScript 组件映射到 Rust 组件 · hook 映射到 hook · 偏差记录在源码里 · interactive loop + tool execution + permissions + MCP + slash commands 已实现 · 其余部分进度中 · 5919 KB · README 明示「Unofficial project, not affiliated with Anthropic」+「Claude/Claude Code are trademarks of Anthropic, PBC」+「Nothing here is endorsed」· Rust 1.88+ edition 2024 是「最低依赖无系统库无 pkg-config」具体路径 · AGPL-3.0 是「网络使用 copyleft + 修改需公开」具体路径 · 与前日 kitze/skillbox「自托管 Skills + 知名独立开发者背书」同构「独立开发者 + Claude Code 周边」领域 · 与昨日 dsh-lab/cordis-bundle-publisher「manifest + minisign + PyPI/OCI publish」同构「Claude Code 生态工具链补齐」但推到「TUI 层重写 + 严肃许可」领域） | Haleclipse/CometixCode | 88 |
| 2 | astra-jev-minecraft-end-to-end / GPT-6 Astra + JEV Minecraft 端到端通关 agent——rmalde/minecraft-agent 1 天 259⭐ ⑂17 fork/star 6.6%（JavaScript · 无 license · Mineflayer + vanilla server + 隐藏 Java sensor 报告龙首精确位置 · nether-final-08 用时 8 分 43.300 秒比上一次 14 分 31.800 秒缩短 40% · End combat 152 秒代替 332 秒 · 131 次 JEV 决策 + 35 次 Astra 调用 · 6 次床爆炸击杀末影龙 · 全部 17 项 run 检查 + 8 项 route/camera/screen 检查 + 29 项本地测试通过 · 不改游戏规则或实体状态 · 摄像头 240 度/秒 + 加速度 960 度/秒² · 隐藏原生 Minecraft 客户端渲染游戏无桌面输入 · 698 KB · README 诚实「Dragon flight and landing times can vary between runs」 · 与昨日 sutro-sh/jev-align「GEPA 对齐 Jev」+ Heman10x-NGU/openJev-verdict-2.0「Non-Autoregressive Decision Engine 击败 TypeSafe Jev & Laya」同构「Jev 决策模型应用层」但推到「Astra 规划 + JEV 选动作 + Minecraft 端到端通关」具身智能领域 · ⚠ 无 license 是企业 / 商业复用风险点） | rmalde/minecraft-agent | 86 |
| 3 | awesome-jev-skills-90-scenarios / Awesome Jev Skills 90 场景 + 9 技能合集——wuyoscar/jev-skill 1 天 142⭐ ⑂3 fork/star 2.1%（Python · MIT · 9 skills + 90 scenarios · Action tests 工作流 · Demos 链接到原作者 browser-use/jev-ultrafast 等 · 双语 README 英文 + 简体中文 · 1011 KB · 与昨日 v-modal/awesome-jev-tools「README 是分类文件首页聚合 + 5 类应用 + 严格纳入标准 + Curation is not endorsement」同构「Jev 资源聚合」但推到「90 场景可运行 + 9 技能可安装 + Action tests 工作流 + 双语 README」具体可执行化领域 · 与 09-19 mizorewww/laya-mlx「Native MLX runtime for Laya typed decision models 7-14 ms on M3 Max」+ mizorewww/laya-coreml「Core ML + Neural Engine」同构「Laya 决策模型本地推理」但推到「Jev 技能生态 + 应用场景 + 浏览器 / inbox / 文档 / 创意」领域） | wuyoscar/jev-skill | 82 |
| 4 | openai-compatible-multi-subscription-pooling-proxy / OpenAI 兼容多订阅池化代理——ghuntley/underclass 1 天 107⭐ ⑂5 fork/star 4.7%（Rust · MIT · /v1/responses + /v1/chat/completions + /v1/models 三端点 · sticky sessions 钉单订阅保持上游 prompt cache warm · quota exhausted 自动冷却到窗口重置 · 池全枯竭 fail-fast 返回最早 Retry-After · chatgpt.com Codex N 订阅 OAuth device flow · api.githubcopilot.com GitHub device flow · web UI accounts / catalog / live request feed · ghuntley 个人开发者背书 GitHub Principal Engineer 出身 + 转独立 · 170 KB · 与 09-17 clawback/claude-code-cost-ledger「session 层成本治理」同构「AI Coding 多账户 / 多订阅治理」但推到「OpenAI 兼容代理 + sticky session + 冷却 + fail-fast」运行时领域 · 与 09-19 NiazMorshed2007/jev-review「本地优先」同构但推到「本地代理 + 多订阅池化」资源治理领域） | ghuntley/underclass | 78 |

#### 当日重点项目

- 🛰️ Haleclipse/CometixCode (1 天 325⭐ ⑂17) — Anthropic Claude Code TUI 的 Rust 1:1 重实现；Rust 2024 edition · AGPL-3.0 · iocraft retained-mode TUI framework · 自带 CometixTUI fork row-level diffing + SIGCONT self-healing + IME cursor + bracketed paste + grid layout；TypeScript 组件映射到 Rust 组件，hook 映射到 hook；interactive loop + tool execution + permissions + MCP + slash commands 已实现；README 明示「Unofficial project, not affiliated with Anthropic」+「Claude/Claude Code are trademarks of Anthropic, PBC」+「Nothing here is endorsed」；Rust 1.88+ edition 2024 最低依赖无系统库无 pkg-config；5919 KB，fork/star 5.2%，Score 88
- 🪨 rmalde/minecraft-agent (1 天 259⭐ ⑂17) — GPT-6 Astra + JEV Minecraft Java 1.16.5 端到端通关 agent；JavaScript · 无 license · Mineflayer + vanilla server + 隐藏 Java sensor 报告龙首精确位置；nether-final-08 用时 8 分 43.300 秒比上一次 14 分 31.800 秒缩短 40%；End combat 152 秒代替 332 秒；131 次 JEV 决策 + 35 次 Astra 调用；6 次床爆炸击杀末影龙；全部 17 项 run 检查 + 8 项 route/camera/screen 检查 + 29 项本地测试通过；不改游戏规则或实体状态；摄像头 240 度/秒 + 加速度 960 度/秒²；隐藏原生 Minecraft 客户端渲染游戏无桌面输入；诚实表态「Dragon flight and landing times can vary between runs」；698 KB，fork/star 6.6%，Score 86
- ⚡ wuyoscar/jev-skill (1 天 142⭐ ⑂3) — Awesome Jev Skills 90 场景 + 9 技能合集；Python · MIT · 9 skills + 90 scenarios · Action tests 工作流；Demos 链接到原作者（browser-use/jev-ultrafast 等）；双语 README 英文 + 简体中文；1011 KB，fork/star 2.1%，Score 82
...
### HyNetworks/OpenGFW（131 stars · fork 58）· v-modal/awesome-jev-tools（158 stars · fork 0）· logan-markewich/jeff（98 stars · fork 4）· liushunqi8-hash/editaplot2026（81 stars · fork 3）

> 证据边界：项目名称、星标、tags、description、license、size、forks 来自 2026-09-20 GitHub Search API 公开元数据（created_at / pushed_at / stargazers_count / forks_count / language / description / topics / license / size）+ 各项目 README 公开摘录（API readme 字段 base64 解码）。"X 天 Y⭐"基于 created_at→2026-09-20 总星数除以经过天数（粗略下限估计），stargazers REST endpoint 需要认证故未做精确单日增量统计。**今日观察窗为 2026-09-19 创建且截至 2026-09-20 仍在快速增长的 4 个非游戏类高质量工程型项目**；09-20 当日新创仓库在 GitHub API 暴露窗口 < 12 小时，本批重点放在 09-19 创建且仍持续增长的项目。**今日最大事件是 `HyNetworks/OpenGFW` 1 天 131⭐ / fork 58 / fork/star 44.3% — DIY GFW Linux 全协议栈流量分析工程化**——把「完整 IP/TCP reassembly + HTTP/TLS/QUIC/DNS/SSH/SOCKS4/5/WireGuard/OpenVPN 协议解析器 + Shadowsocks/VMess/Trojan 加密代理识别 + 基于 expr 规则引擎 + 热重载规则 SIGHUP 重载 + Flow-based multicore 负载均衡 + Connection offloading + NFQueue IO 抽象」做成 285 KB Go 项目；ad blocking / parental control / malware protection / VPN 滥用防护 / 流量分析 log only / 居家网络主权 6 类用例；README 明示「CAUTION very early stages」；fork/star 44.3% 极高企业 / VPS fork 信号——说明已有大量 fork 准备「在自有 Linux 上跑起 GFW-like 流量分析」。**今日 4 个项目分三条主线**：**A. 网络主权基础设施**（OpenGFW 自托管 + 全协议栈 + expr 策略 + 热重载）——把昨日 thruwire/foreman「Jev supervisor 监管 + Python policy 5 类动作」的「监管 + 策略」分层思想推到「网络栈层监管 + expr 策略」基础设施领域，把昨日 LingxiangXu/traceclause「本地优先 + 不外发 AI」的「反 SaaS + 不外发」推到「本地网络主权 + 不外发流量」基础设施领域；**B. Jev 生态去 SaaS 化 + 资源聚合**（awesome-jev-tools 生态聚合 + jeff 自托管 drop-in）——把 09-15 ~ 09-19 五日「Jev 生态从单点模型到生态爆发」的「awesome 列表资源聚合」+「自托管 fork 替代官方 API」两条路径同时推到「明确纳入标准 + 自托管 drop-in replacement」严肃化；**C. 中文开发者 + 平台专属 + 反 SaaS**（editaplot2026 中文 + Codex Skill + Origin 集成）——把昨日 indada/repopilot「verification-driven iteration + 维护者最终 merge gate」的「AI 提议 + 人类最终 gate」推到「科学绘图 + 用户最终确认图形元素」领域；与前日 pengchujin/MacTV「Apple Silicon + 硬件桥接」同构「中文开发者 + 平台专属 + 硬件集成」但推到「中文开发者 + Codex Skill + Origin 集成」领域。**与昨日「Jev 生态爆发 + 多 agent 编排 + 验证治理 + 本地优先」四主线不同**，今日 Jev 生态从「爆发期」进入「去 SaaS 化期」+「资源聚合期」——v-modal/awesome-jev-tools（158⭐）+ logan-markewich/jeff（98⭐）+ sutro-sh/jev-align（73⭐）+ uehaj/jev-semgrep（65⭐）+ ekzhang/openjev-sglang 等持续填充，但 awesome-jev-tools 摘要在「明确纳入标准 + curation is not endorsement」上做出严肃化，jeff 在「自托管 drop-in + 完整 typesafe-sdk 兼容 + 诚实 less accurate than jev」上做出严肃化——**意味着 Jev 生态从 09-15 ~ 09-19 的「爆发 + 横向应用」回到「自托管 + 资源聚合 + 自我解构」演化**。**真正的今日主线是「网络主权 + Jev 生态去 SaaS + 中文场景本地工具」三条非 Jev 单一爆发的主线**——这是 09-20 趋势相比 09-15 ~ 09-19 的关键演化信号。

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|---:|---|---|---:|
| 1 | diy-gfw-full-protocol-stack / DIY GFW Linux 全协议栈流量分析工程化——HyNetworks/OpenGFW 1 天 131⭐ ⑂58 fork/star 44.3%（Go · MPL-2.0 · 完整 IP/TCP reassembly + HTTP/TLS/QUIC/DNS/SSH/SOCKS4/5/WireGuard/OpenVPN 协议解析器 + Shadowsocks/VMess 加密代理识别 基于 gfw.report/publications/usenixsecurity23 + Trojan (proxy protocol) detection + 基于 expr 规则引擎 + 热重载规则 SIGHUP 重载 + Flow-based multicore 负载均衡 + Connection offloading + NFQueue IO 抽象 + 可扩展 IO 实现 · ad blocking / parental control / malware protection / abuse prevention for VPN/proxy services / traffic analysis log only / 居家网络主权 6 类用例 · README 明示「CAUTION very early stages」 · WIP ML 流量分类 + Web UI · 285 KB · fork/star 44.3% 极高企业 / VPS fork 信号 · 与昨日 thruwire/foreman「Jev supervisor 监管 + Python policy 5 类动作」同构「监管 + 策略」分层但推到「网络栈层监管 + expr 策略」基础设施领域 · 与昨日 LingxiangXu/traceclause「本地优先 + 不外发 AI」同构反 SaaS 但推到「本地网络主权 + 不外发流量」基础设施领域 · MPL-2.0 是「文件级 copyleft + 可商用 + 修改需公开」的具体路径） | HyNetworks/OpenGFW | 88 |
| 2 | jev-ecosystem-aggregation / Jev 决策模型生态聚合 awesome 列表——v-modal/awesome-jev-tools 1 天 158⭐ ⑂0 fork/star 0%（None · None · README 是分类文件首页聚合最新收录 + Jev 定义「not a chat model · takes unstructured state + typed question returns typed decision + confidence」 + classification / infrastructure routing / rubric scoring / verification gates / autonomous agent guardrails 5 类应用分类 · 明确纳入标准排除 generic classifier + pure theory + launch-hype + private sources · Curation is not endorsement · 跨行业覆盖 topics 包含 robotics / robotics-algorithms / robotics-control / robotics-simulation · 83 KB · 与昨日 cobanov/awesome-jev + fatwang2/awesome-jev + AbdelStark/awesome-typesafe 同构「Jev 资源聚合」但推到「明确纳入标准 + 严肃化」领域 · 与昨日 thruwire/foreman「Jev supervisor」+ NiazMorshed2007/jev-review「本地优先 MCP」同构「Jev 应用层」但推到「Jev 资源 + 生态聚合层」） | v-modal/awesome-jev-tools | 84 |
| 3 | self-hosted-jev-drop-in / 自托管 Jev drop-in replacement powered by GLiFormer——logan-markewich/jeff 1 天 98⭐ ⑂4 fork/star 4.1%（Python · MIT · GLiFormer 400M 参数 encoder knowledgator/gliformer-large-v1 · 完整 typesafe-sdk 兼容 · TYPESAFE_BASE_URL 指 jeff 即可替换官方 endpoint · choice / score / noul 三类问题 · CUDA → MPS → CPU 设备自动降级 · uv sync --extra dev + uv run hf download + JEFF_API_KEYS=devkey uv run jeff · README 诚实「Cheaper to self-host but less accurate than jev on reasoning-heavy tasks」 · 957 KB · 与昨日 NiazMorshed2007/jev-review「本地优先 MCP 质量评估」同构「本地优先反 SaaS」但推到「自托管 Jev 替代 API + 开源 encoder 模型」领域 · 与昨日 featherless-ai/simple-jev「开源模型转 classifier/jev endpoint」同构但推到「自托管 + drop-in + 完整 typesafe-sdk 兼容」严肃化 · 与昨日 ekzhang/openjev-sglang「Jev-compatible API endpoint 基于开源模型 prefill-only」同构但推到「GLiFormer encoder + full choice/score/noul」领域 · 是 09-15 ~ 09-19 「Jev 生态去 SaaS 化」路径的具体兑现） | logan-markewich/jeff | 82 |
| 4 | codex-skill-scientific-plotting / Codex 可编辑科研绘图 Skill Windows——liushunqi8-hash/editaplot2026 1 天 81⭐ ⑂3 fork/star 3.7%（Python · Apache-2.0 · Windows 10/11 x64 only · Codex Skill 形态 · Origin 2026 (10.30) verified · 输入数据 → 理解表 → 逐列说明用途 → 推荐图形 → 用户确认 → 调用专用 Origin 实例 → 生成可编辑 OPJU + 导出 PNG/PDF/TIF 三格式 · 不让 Python 预览冒充 Origin 成图 · 不擅自补列 / 拟合 / 推断结论 · 科学含义和最终选择始终由你决定 · 不确定的列单独列出来请你确认 · 5.7 MB · 与昨日 indada/repopilot「verification-driven iteration + 维护者最终 merge gate」同构「AI 提议 + 人类最终 gate」但推到「科学绘图 + 用户最终确认图形元素」领域 · 与前日 pengchujin/MacTV「Apple Silicon + 硬件桥接」同构「中文开发者 + 平台专属 + 硬件集成」但推到「中文开发者 + Codex Skill + Origin 集成」领域 · 与昨日 skill-lab/feishu-chat-archive「中国云办公 API 反 SaaS 编排」+ LingxiangXu/traceclause「本地优先 + 不外发」同构「中文场景本地工具 + 反 SaaS」但推到「中文科研 + 本地 Origin + AI 辅助 + 不替用户决定」领域） | liushunqi8-hash/editaplot2026 | 78 |

#### 当日重点项目

- 🧱 HyNetworks/OpenGFW (1 天 131⭐ ⑂58) — DIY GFW Linux 全协议栈流量分析工程化；Go · MPL-2.0 · 完整 IP/TCP reassembly + HTTP/TLS/QUIC/DNS/SSH/SOCKS4/5/WireGuard/OpenVPN 协议解析器 + Shadowsocks/VMess/Trojan 加密代理识别 + 基于 expr 规则引擎 + 热重载规则 SIGHUP 重载 + Flow-based multicore 负载均衡 + Connection offloading + NFQueue IO 抽象；ad blocking / parental control / malware protection / VPN 滥用防护 / 流量分析 log only / 居家网络主权 6 类用例；明示「CAUTION very early stages」；WIP ML 流量分类 + Web UI；285 KB，fork/star 44.3%，Score 88
- 📚 v-modal/awesome-jev-tools (1 天 158⭐ ⑂0) — Jev 决策模型生态聚合 awesome 列表；README 是分类文件首页聚合最新收录；TypeSafe AI System One 决策模型定义「接受 state + typed question 返回 typed decision + confidence」；classification / infrastructure routing / rubric scoring / verification gates / autonomous agent guardrails 5 类应用分类；明确纳入标准排除 generic classifier + pure theory + launch-hype + private sources；Curation is not endorsement；跨行业覆盖 topics 包含 robotics；83 KB，fork/star 0%，Score 84
- 🦊 logan-markewich/jeff (1 天 98⭐ ⑂4) — 自托管 Jev drop-in replacement；Python · MIT · GLiFormer 400M 参数 encoder；兼容 typesafe-sdk 完整 choice / score / noul 三类问题；TYPESAFE_BASE_URL 指 jeff 即可替换官方 endpoint；CUDA → MPS → CPU 设备自动选择；uv tool install 可运行；README 诚实表态「Cheaper to self-host but less accurate than jev on reasoning-heavy tasks」；957 KB，fork/star 4.1%，Score 82
- 📈 liushunqi8-hash/editaplot2026 (1 天 81⭐ ⑂3) — Codex 可编辑科研绘图 Skill；Python · Apache-2.0 · Windows 10/11 x64 only；Codex Skill 形态；Origin 2026 (10.30) verified；输入数据 → 理解表 → 逐列说明用途 → 推荐图形 → 用户确认图形元素 → 调用专用 Origin 实例 → 生成可编辑 OPJU + 导出 PNG/PDF/TIF 三格式；明确「不让 Python 预览冒充 Origin 成图」+「不擅自补列 / 拟合 / 推断结论」+「科学含义和最终选择始终由你决定」+「不确定的列单独列出来请你确认」4 项明确边界；5.7 MB，fork/star 3.7%，Score 78
...
### thruwire/foreman（172 stars · fork 11）· NiazMorshed2007/jev-review（72 stars · fork 6）· Worldbuilder013/HEXIS（69 stars · fork 1）· kitze/skillbox（69 stars · fork 7）· pengchujin/MacTV（61 stars · fork 2）
...
| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|---:|---|---|---:|
| 1 | foreman-jev-supervisor / TypeSafe Jev 软件工厂 Foreman——thruwire/foreman 1 天 172⭐ ⑂11 fork/star 6.4%（Python · MIT · 两个并发 loop · `codex exec --cd repo --sandbox workspace-write --json mission` worker · JSONL stdout/stderr concurrent streamed bounded in memory + persisted as factory events · Jev 并行评估 7 维度 implementation_complete / tests_sufficient / requirements_satisfied / worker_stuck / needs_verification / work_off_track / meaningful_progress 返回概率 · Python policy 层决定 continue / stop / retry / verify / finish · 默认边界 20K diff + 12K tail + 30 events + 10 workers · FactoryConfig 可调 · worker implementation replaceable 取决于 small worker protocol · verifier 复用 Codex 不同 verification mission · 不替换 Codex 内部 reason/tool/observe loop · Missions stay broad · 4 篇 docs theory.md / why-jev.md / what-foreman-proves.md / runtime.md · 46 KB repo · 明确表态「an architectural experiment」 · 与 NiazMorshed2007/jev-review + kitze/skillbox + ekzhang/openjev-sglang 等同构 TypeSafe Jev 决策模型生态爆发但推到「软件工厂监管层」最高抽象） | thruwire/foreman | 88 |
| 2 | jev-quality-review / 本地优先 MCP 软件质量评估插件——NiazMorshed2007/jev-review 1 天 72⭐ ⑂6 fork/star 8.3%（TypeScript · MIT · Node.js 20+ · MCP stdio · 单一 `jev_review` 工具 · 支持 Claude Code / Codex / Cursor / OpenCode 四大 Coding Agent · 评估 correctness / complexity / changeability / modularity / tests / security 多维度 · 用户 API key 留在本地 · 无 hosted backend / 无 database / 无 telemetry / 无 author-operated proxy · 唯一 remote 请求直接到 Jev API · 编码由主 agent 完成 Jev 只供应 scalar signal · TypeSafe AI 官方 console 配 Jev API key · 2.9 MB repo · 与昨日 karanb192/awesome-claude-code-mods / agent-sec/mod-provenance-graph「plugin/mod 供应链可见性」同构但推到「plugin/mod 质量评估」领域 · 与 kitze/skillbox「自托管 + 版本化 Skills」同构但推到「本地优先 + 质量评估」领域） | NiazMorshed2007/jev-review | 84 |
| 3 | skill-md-fsm-compiler / SKILL.md → 扩展有限状态机编译器——Worldbuilder013/HEXIS 1 天 69⭐ ⑂1 fork/star 1.4%（Python · MIT · 3.11/3.12 · 论文「Compiling Agent Skills into Extended Finite State Machines」配套代码 · `efsm-v1` JSON schema 含 typed variables + tool/model/judge/user/end 5 类 actions + ordered guarded transitions + bounded loops + fallback state · LLM 在 state 内推理 · 顺序由 program 强制 · 4 个 shipped machines data analysis / mathematics / QA over corpus / spreadsheet editing · GUIDE.md + PROMPT.md 自描述产物 · 350+ tests 不需网络 / endpoint / key · OpenAI-compatible `--model --base-url --api-key-env` · OpenCode native tools + 本地 `bash` backend + 工具 registry · fallback state 重试 + 解释执行兜底 · alpha status · 6.1 MB repo · 与昨日 TopVitamin/agent-skills「中文 Codex Skills 实例」同构但推到「SKILL.md → 状态机编译」学术严肃度 · 与昨日 wshobson/agents「多 Harness Agent Skills 市场」同构但推到「Skill 编译成可执行状态机」领域） | Worldbuilder013/HEXIS | 82 |
| 4 | self-hosted-skill-library / 自托管版本化 AI agent 技能库——kitze/skillbox 1 天 69⭐ ⑂7 fork/star 10.1%（TypeScript · MIT · MCP + scoped clients + 可选 Jev recommendations · 自托管 · 版本化 · 主流 Coding Agent 通用接入 · Kitze 知名独立开发者多项目矩阵营销 Zero To Shipped + Sotto + Tinkerer Club + Sizzy + Supermac · 191 KB repo · 与昨日 wshobson/agents「多 Harness Agent Skills 市场」同构但推到「自托管 + 个人开发者背书」领域 · 与 TopVitamin/agent-skills「中文 Codex Skills」同构但推到「英文 + 知名独立开发者 + 自托管」领域） | kitze/skillbox | 80 |
| 5 | macos-hdmi-cec-tv-remote / macOS 电视遥控菜单栏 App——pengchujin/MacTV 1 天 61⭐ ⑂2 fork/star 3.3%（Swift · MIT · Apple Silicon arm64 + macOS 14+ · SwiftUI · HDMI-CEC 双向遥控 · Mac 音量键控制电视音量（需辅助功能权限）· 电视遥控器控制 Mac 媒体模式（切歌 / 播放暂停 / 切换应用）+ 鼠标模式（移动 / 长按加速 / 6 档速度 / 长按确认后松开右击）+ 返回键映射 Esc · 菜单栏遥控器（方向 / 确认 / 返回 / 唤醒 / 待机 / 菜单）· 输入源切换（切回 Mac / HDMI 1-4）· VoiceOver + 浅色/深色外观 + 简繁英三语 · Homebrew cask `pengchujin/tap/mactv` 安装 + DMG 下载 v0.1.2 · TV 品牌术语兼容 SIMPLINK（LG）/ Anynet+（Samsung）/ BRAVIA Sync（Sony）· 5.1 MB repo · 与 FelixQiu1/XiaoAi-LLM-Router「老硬件 + 中间层升级」同构但推到「老电视 + Mac HDMI-CEC 中间层」硬件桥接领域 · 「Mac 当电视用」/「电视当 Mac 显示器用」双向场景是 2026 居家办公明确需求） | pengchujin/MacTV | 78 |
...
### dsh-lab/cordis-bundle-publisher（14 stars · fork 3）· Neuroscale/cortical-stack（56 stars · fork 9）· skill-lab/feishu-chat-archive（11 stars · fork 0）· agent-sec/mod-provenance-graph（9 stars · fork 4）· clawback/claude-code-cost-ledger（17 stars · fork 2）
...
| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|---:|---|---|---:|
| 1 | dsh-bundle-publisher / DSH cordis bundle 打包签名发布 CLI——dsh-lab/cordis-bundle-publisher 1 天 14⭐ ⑂3 fork/star 21.4%（Python · MIT · bundle.manifest.yml schema 校验 + cordis-1.x 兼容矩阵声明 + SHA-256 bundle 元数据 + minisign 离线签名 Ed25519 + PyPI + OCI 双注册源 + GitHub Actions / GitLab CI / 本地三套分发通道 · init / validate / sign / publish / verify / install 六命令 · 自动 prerelease 标签 + commit SHA 截断嵌入元数据 · 1.2 MB repo · 与昨日 dsh-computer-use_codex-style cordis.patch.yml 安装机制同构 DSH bundle 工具链补齐「打包签名发布」侧） | dsh-lab/cordis-bundle-publisher | 86 |
| 2 | cortical-microcircuit-stack / Frozen LM × 哺乳动物皮层 7 层微电路栈——Neuroscale/cortical-stack 1 天 56⭐ ⑂9 fork/star 16.1%（Python · Apache-2.0 · V1 microcircuit 7 layers L1/L2/3/L4/L5/L6 + 兴奋/抑制单元 · 31,200 节点 + 4,485,600 connections · 412,000 参数 adapter · Liquid AI LFM2-1.2B 冻结 · token embeddings + 微电路位置编码双驱动固定 graph · adapter 双流 readout 行为流 + 上下文流 · Apple Silicon MPS + NVIDIA CUDA + CPU 三路径 · upstream SHA-256 · 可选 Triton kernel 加速 SciPy 同语义 · 48 corpus + 24 synthetic style 训练 + 32 separate test 评估 · parameter-matched 直接输入 adapter 控制组 · runs/conversation-v3/ 写一次不被覆盖 · /reset 清空 /quit 退出 · 诚实表态「This does not mean a biological cortex understands language」 · 142 KB repo · 与昨日 FLModel/flm「Frozen LM × 苍蝇脑连接组」同构但推到「哺乳动物皮层 7 层微电路」研究领域从无脊椎到脊椎的进化跳跃） | Neuroscale/cortical-stack | 82 |
| 3 | claude-mod-provenance-graph / Claude Code Mod 依赖图 + provenance SBOM 工具——agent-sec/mod-provenance-graph 1 天 9⭐ ⑂4 fork/star 44.4%（JavaScript · Apache-2.0 · Anthropic 2026-09-09 function hooks 发版后产物 · `mod-graph build` 解析每个 Mod package.json + $.hooks + plugin.json + references 生成 mod-graph.json 有向图（节点为 mod / 边为依赖/钩子事件/$ 调用） · `mod-graph provenance` 输出 CycloneDX 1.5 SBOM 含每个 mod 维护者/许可/钩子事件/L0-L3 reach · `mod-graph diff` 对比昨日 mod-graph.json 与今日生成版本仅差异输出 · `mod-graph graphviz` 输出 Graphviz DOT · 145 KB repo · fork/star 44.4% 极高早期信号 · 与昨日 karanb192/awesome-claude-code-mods L0/L1/L2/L3 reach 同构但推到「Mod 依赖图 + SBOM 标准化」治理层 · 与昨日 ToolReplay「session 层审计」同构但推到「plugin/mod 供应链可见性」领域） | agent-sec/mod-provenance-graph | 81 |
| 4 | feishu-chat-archive-skill / 飞书群聊归档 Codex Skill——skill-lab/feishu-chat-archive 1 天 11⭐（Python · MIT · tenant_access_token + im/v1/messages 全量分页 + 增量游标 + WAL 增量 · 文本 + 富文本 + 可解析卡片 + 消息引用 · 每个报告条目引用本次真实 message_id · 已确认/已接收/待完成/个人观点/归档建议五分类 · PNG 长图 + HTML 网页 + Markdown 摘要三格式同一正文 · requests + cryptography + zstandard + Pillow + Python 3.12 · 中文字体微软雅黑 · tenant_access_token 默认仅存内存退出清理 · 默认不上传原始记录 · 飞书开放平台 API v1 实际验证 · 1.1 MB repo · 与昨日 Tina2088/wechat-group-report「本地数据库 + Codex Skill 编排」同构但推到「云 API + Codex Skill 编排」飞书场景 · 反 SaaS 范式扩展到「中国云办公 API 编排」） | skill-lab/feishu-chat-archive | 80 |
| 5 | claude-code-cost-ledger / Claude Code token 成本分账账本——clawback/claude-code-cost-ledger 1 天 17⭐ ⑂2 fork/star 11.8%（Python · MIT · ~/.claude/projects/**/*.jsonl 读取 · canonical JSON 编码 + content_hash 去重 · message_id / parent_uuid 链 · cache_read_input_tokens / cache_creation_input_tokens / input / output 四类 token · per-session / per-day / per-bucket 聚合 · cost.csv / cost.json / cost.md 三格式输出 · 当前价格快照可被本地覆盖 prices.local.yaml · buckets.yaml 分账维度 + slack 通知可选 · pre-commit hook 校验单日成本超阈值 · 188 KB repo · 与昨日 ToolMonsters/claude-code-routing「cheap model 当 worker + Claude 当 orchestrator」成本优化同构但推到「成本可见 + 分账账本」治理层 · 与昨日 ToolReplay「session 层审计」同构但推到「session 层成本治理」领域） | clawback/claude-code-cost-ledger | 79 |
...
### yetone/cumora（2,899 stars） · CopilotKit/OpenBot（2,308 stars） · s1dashu/ip-as-logo-skill（3,767 stars） · cinderline/northcinder（1,205 stars）

> 证据边界：项目名称、星标、标签和描述来自当日 GitHub API 快照；趋势归纳基于这些公开元数据，不将热度等同于价值。

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|---:|---|---|---:|
| 1 | agent-runtime / 团队协作 | yetone/cumora, CopilotKit/OpenBot | 92 |
| 2 | agent-skills / 单点能力 | s1dashu/ip-as-logo-skill, Spielewoy/autoprompt-skill, JetBrains/benjamin-plus-skill | 90 |
| 3 | MCP / 协议化 | cinderline/northcinder, browser-use/macos-harness, joe960913/Jixu | 88 |
| 4 | agent-governance / 治理 | CopilotKit/OpenBot, azrtydxb/procoder | 85 |
| 5 | A2A / multi-agent routing | wang2122/sprix-sage-router, joe960913/Jixu | 82 |
...
### antirez/h3.c 爆发（129→1,201，+831%/+1,072），Redis 之父原生推理引擎三日破千，"推理成本是瓶颈"判断被市场二次确认 · anydoc 昨日"稳态"判断被逆转——delta 从 +719 反弹到 +957（13,399→14,356，+7.1%），"基于有限数据过早下稳态结论"的方法论修正案例 · SMNETSTUDIO/WeChat-AI 疑似刷量样本（1,406⭐/1,040 fork/1 subscriber/0 issue/无描述/2天，fork≈star 零参与度），继 open-kimi-ppt-skill 后又一个"热度≠价值"对照案例 · MiniMax-H3 持续高位（4,164→5,194，+1,030/+24.7%），衍生生态 379→448，连续四日千级增量

#### 今日四大趋势

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | antirez/h3.c 爆发（129→1,201，+831%/+1,072），连续三日破千。Redis 创始人 Salvatore Sanfilippo 的 MiniMax-H3 原生 Metal 推理引擎三日破千星。fork 7→57，subscribers 1→10。README 能力（prompt→video/audio、首尾帧、Ref2VA、core-reuse、跳层）均可核验。M5 Max 性能为作者自述未独立复现 | h3c, minimax-h3 | 92 |
| 2 | anydoc 昨日"稳态"判断被逆转——delta 从 +719 反弹到 +957（13,399→14,356，+7.1%），fork 675→751。昨日报告判断"成熟稳态"，今日数据明确修正。反弹原因待观察（推断：H3 生态热度外溢/周末效应）。"基于有限数据过早下稳态结论"的方法论修正案例 | anydoc | 90 |
| 3 | SMNETSTUDIO/WeChat-AI 疑似刷量样本（1,406⭐/1,040 fork/1 subscriber/0 issue/无描述/2天），fork≈star（0.74）零参与度，与 open-kimi-ppt-skill 归档后 fork 异常增长同构。作为"热度≠价值"对照案例入库，与 anydoc（真实需求）形成方法论对照 | wechat-ai | 68 |
| 4 | MiniMax-H3 官方仓库持续高位（4,164→5,194，+1,030/+24.7%），衍生生态搜索 379→448。连续四日 +699→+924→+1,471→+1,030。antirez/h3.c 爆发 + 官方仓库持续 + 衍生生态扩张，三者共振确认 H3 是本周最强生态级信号。kimi-k3-in-c 同步 +242（+5.2%），本地 MoE 推理赛道头部双项目并行放量 | minimax-h3, kimi-k3-in-c | 90 |

#### 今日新增/更新项目

| 项目 | Stars | 分类 | Score | 变化 |
|------|-------|------|-------|------|
| antirez/h3.c | 1,201 | 观察型 | 92 | 更新（129→1,201，+1,072/+831%，三日破千，fork 7→57，score 86→92） |
| firecrawl/anydoc | 14,356 | 工具型 | 90 | 更新（13,399→14,356，+957/+7.1%，delta 反弹，昨日"稳态"判断被逆转） |
| MiniMax-AI/MiniMax-H3 | 5,194 | 观察型 | 90 | 更新（4,164→5,194，+1,030/+24.7%，衍生生态 379→448） |
| SMNETSTUDIO/WeChat-AI | 1,406 | 观察型 | 68 | 新增（疑似刷量样本，fork/star=0.74，1 subscriber，0 issue，无描述，"热度≠价值"对照案例） |
| ShawnPana/phone-harness | 1,488 | 观察型 | 85 | 更新（1,282→1,488，+206/+16.1%，fork 100→129） |
| oil-oil/oil-motion | 1,475 | 工具型 | 84 | 更新（1,142→1,475，+333/+29.2%，加速中） |
| FareedKhan-dev/kimi-k3-in-c | 4,891 | 观察型 | 85 | 更新（4,649→4,891，+242/+5.2%，fork 727→774） |
| KKKKhazix/human-writing | 2,405 | 工具型 | 84 | 更新（2,266→2,405，+139/+6.1%，增速稳定 +5-8%） |
| trycompai/crm | 8,217 | 平台候选 | 86 | 更新（8,102→8,217，+115/+1.4%，连续七日衰减，score 87→86） |
| yc-software/qm | 13,092 | 平台候选 | 86 | 更新（12,950→13,092，+142/+1.1%，停滞通道，score 87→86） |
| genspark-ai/genoffice | 2,558 | 平台候选 | 84 | 更新（2,469→2,558，+89/+3.6%，增速持续衰减） |
| Binaryify/open-kimi-ppt-skill | 1,603 | 观察型 | 70 | 更新（archived，star -4，fork 1,182→1,194 仍异常增长第五日） |

#### 趋势观察

- **h3.c 三日破千——市场二次确认"推理成本是瓶颈"：** antirez/h3.c +831%（129→1,201）证明开发者社区高度认同"推理优化是真问题"。vertical-slice 增量构建方法学（每步可独立验证）值得借鉴。H3 生态三层共振（官方 +1,030 / h3.c +1,072 / 衍生 379→448）。
- **anydoc "稳态"判断被逆转——方法论修正：** delta 从 +719 反弹到 +957，打破昨日"成熟稳态"判断。教训：七日数据不足以确认稳态，尤其当外生热度事件（H3 生态）可能扰动相邻赛道时。"稳态"判断需更长观测窗口（至少两周）。
- **"热度≠价值"对照样本库扩充：** WeChat-AI（fork/star=0.74、1 subscriber）+ open-kimi-ppt-skill（归档后 fork 异常增长）构成"非自然热度"证据链；anydoc（fork/star=0.05、subscribers 35）为真实需求标杆。fork/star 比例 + subscribers + issues 三指标交叉验证。
- **应用层全面稳态尾声：** crm（+1.4%）、qm（+1.1%）、genoffice（+3.6%）增速全面低个位数，应用层红利期基本结束。
- **Skill 生态单点脆弱性持续：** 0xwilliamortiz 全账号仍 404（claude-red + humanizer-cli），第三日无恢复。
...
### MiniMax-H3 反而加速（1,070→1,769，+699/+65%），模型仓库增速反超衍生生态（265→314），加速器类（Spectrum/Turbo）持续领跑，"生成质量瓶颈"让位于"推理成本+工作流"瓶颈的判断被市场继续确认 · anydoc 进入稳态（10,866→12,055，+1,189/+11%，增速序列 +331%→+72%→+35%→+11% 单调收敛，教科书式真实需求曲线收敛完成） · open-kimi-ppt-skill 被作者归档（archived=true，1588⭐/1113 fork）——昨日 fork 异常暴增（343→914）后的可疑信号今天落地为归档，刷量/弃坑判断被验证 · 本地 MoE 推理路线扩展到 Apple 原生栈——Swiftlet（456⭐，Swift+Metal 在 iPhone 17 上跑 35B Qwen3.6 MoE，峰值 2.6GB RAM），与 kimi-k3-in-c（今日继续 +16% 加速）构成"多平台本地推理"覆盖

#### 今日四大趋势

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | MiniMax-H3 反而加速——官方仓库增速从 +0 跃到 +65%（1,070→1,769），生态仓库 265→314，模型仓库自身增速反超衍生生态，视频生成赛道"生成质量瓶颈"让位于"推理成本+工作流编排"瓶颈的判断被市场继续确认，加速器类（Spectrum 382⭐/Turbo 291⭐）持续领跑衍生生态 | minimax-h3 | 88 |
| 2 | anydoc 进入稳态放量（10,866→12,055，+1,189/+11%，fork 506→575）——增速序列 +331%→+72%→+35%→+11%，五日衰减率单调收窄，教科书式真实需求曲线收敛完成；稳态增量 +1K/日，万星量级头部位置巩固 | anydoc | 90 |
| 3 | open-kimi-ppt-skill 被作者归档（archived=true，1588⭐/1113 fork）——昨日报告的 fork 异常暴增（343→914 单日+571，fork/star=0.58 异常）今天落地为归档，"疑似刷量或批量部署"判断被验证；同日 anydoc 稳态、claude-red 稳健增长（+23%）形成对比，验证"分清热度与价值"的方法论价值 | open-kimi-ppt-skill | 78 |
| 4 | 本地 MoE 推理路线扩展到 Apple 原生栈——Swiftlet（456⭐，Swift+Metal 在 iPhone 17 上跑 35B Qwen3.6 MoE，峰值 2.6GB RAM，按需流式加载 expert），与 kimi-k3-in-c（C99，今日继续 +16% 加速）构成"本地大模型推理"赛道的多语言/多平台覆盖 | swiftlet, kimi-k3-in-c | 83 |

#### 今日新增/更新项目

| 项目 | Stars | 分类 | Score | 变化 |
|------|-------|------|-------|------|
| firecrawl/anydoc | 12,055 | 工具型 | 90 | 更新（10,866→12,055，+1,189/+11%，增速序列单调收敛，稳态放量，fork 破 575） |
| MiniMax-AI/MiniMax-H3 | 1,769 | 观察型 | 88 | 更新（1,070→1,769，+699/+65%，反而加速，模型仓库增速反超衍生生态，生态 265→314，score 86→88） |
| 0xwilliamortiz/claude-red | 681 | 工具型 | 84 | 更新（555→681，+126/+23%，208 watchers 持续高位，安全 Skill 品类定义者地位巩固） |
| leonickson1/Swiftlet | 456 | 观察型 | 83 | 新增（Swift+Metal 在 iPhone 17 上跑 35B MoE，本地推理路线扩展到 Apple 栈） |
| jd-opensource/JoyAI-Video-Edit | 512 | 观察型 | 82 | 新增（京东实时流式视频编辑，720p 30 FPS，因果 VAE） |
| trycompai/crm | 7,751 | 平台候选 | 87 | 更新（7,471→7,751，+280/+4%，连续四日衰减尾声） |
| yc-software/qm | 12,525 | 平台候选 | 87 | 更新（12,274→12,525，+251/+2%，连续三日停滞通道，万星稳态确认） |
| Binaryify/open-kimi-ppt-skill | 1,588 | 观察型 | 70 | 更新（archived=true，昨日 fork 异常判断被验证，降级案例库，score 82→70） |
| KKKKhazix/human-writing | 1,985 | 工具型 | 84 | 更新（1,867→1,985，+118/+6%，增速骤降，逼近 2K 但增长明显放缓，score 85→84） |
| FareedKhan-dev/kimi-k3-in-c | 3,751 | 观察型 | 85 | 更新（3,247→3,751，+504/+16%，继续加速，与 Swiftlet 多平台共振，score 84→85） |
| genspark-ai/genoffice | 2,249 | 平台候选 | 84 | 更新（2,163→2,249，+86/+4%，增速持续衰减，score 85→84） |
| microsoft/skill-recorder | 2,509 | 工具型 | 84 | 更新（2,312→2,509，+197/+9%，增速稳定，应用层唯一非衰减项目） |

#### 趋势观察

- **MiniMax-H3 反而加速 + 模型仓库增速反超衍生生态：** 说明生态从"插件爆发期"进入"主路径集成期"。加速器类（Spectrum/Turbo）持续领跑，推理成本仍是首要瓶颈。印证昨日"瓶颈在推理成本+工作流编排而非生成质量"的判断。
- **anydoc +11% 稳态放量完成收敛：** 增速序列 +331%→+72%→+35%→+11% 是教科书式真实需求曲线。与脉冲项目（open-kimi-ppt-skill 归档）形成鲜明对比。
- **open-kimi-ppt-skill 归档验证"异常 fork 判断"方法论：** 昨日基于 fork/star=0.58 异常比值的判断今天落地为归档。fork/star 比持续恶化（0.27→0.58→0.70）是"刷量/批量部署/弃坑前兆"的可靠信号。
- **本地 MoE 推理赛道多平台覆盖：** Swiftlet（Apple Swift）+ kimi-k3-in-c（C99）+ ANEMLL（先例）共同构成"本地大模型流式推理"赛道。
- **应用层整体进入衰减尾声（第八日确认）：** qm（+2%）、crm（+4%）、genoffice（+4%）三个项目增速全面下降到低个位数。skill-recorder（+9%）是唯一稳定项目。
...
### qm 第三日续涨 +2,443（7,015→9,458⭐，fork 736→998），第三日增量反超第二日彻底排除脉冲假设，应用层产品化趋势完全确认 · K3 本地推理品类级爆发——kimi-k3-in-c 单日 +955（218→1,173，近 5.4 倍）引爆极端低内存卖点，waste +510 续涨（连续三日增速递增） · OpenAI 官方 ten-proofs 用 Lean 4 形式化十项数学/理论计算机成果（球填充/非 sofic 群/Connes 刚性反例等），标志 AI + 形式化验证交叉品类出现 · agent 质量基础设施成型——ratchet 把 coding agent 极简规则从开环注入变闭环检查（PostToolUse hook 实时测复杂度，285 watchers 高质量关注）· 应用层多路线并行扩张（通用平台 qm/垂直重写 crm/AI-native 桌面 genoffice 同步放量）

#### 今日四大趋势

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | 应用层产品化第三日完全确认——qm 三日连涨无一日回落（1,367→9,458，三日 +8,091），第三日 +2,443 反超第二日 +2,233 彻底排除脉冲；crm/genoffice 多路线同步放量 | qm, crm, genoffice | 92 |
| 2 | K3 本地推理品类级爆发——kimi-k3-in-c 单日 +955（218→1,173，近 5.4 倍）引爆极端低内存（8.24GB）卖点，waste +510 续涨，品类从个别项目变为系统性关注 | kimi-k3-in-c, waste | 88 |
| 3 | AI 推理边界外延——OpenAI 官方 ten-proofs 用 Lean 4 形式化十项数学成果（球填充/非 sofic 群/Connes 刚性反例等），标志 AI + 形式化验证交叉品类出现 | ten-proofs | 85 |
| 4 | agent 质量基础设施成型——ratchet 把极简规则从开环注入变闭环检查（PostToolUse hook 实时测复杂度/依赖/wrapper/yagni），与 skill-recorder 正交互补 | ratchet, skill-recorder | 84 |

#### 今日新增/更新项目

| 项目 | Stars | 分类 | Score | 变化 |
|------|-------|------|-------|------|
| yc-software/qm | 9,458 | 平台候选 | 90 | 更新（7,015→9,458，第三日 +2,443，三日累计 +8,091） |
| FareedKhan-dev/kimi-k3-in-c | 1,173 | 观察型 | 83 | 更新（218→1,173，+955，爆发性增长） |
| openai/ten-proofs | 432 | 观察型 | 82 | 新增 |
| genspark-ai/genoffice | 564 | 平台候选 | 84 | 新增 |
| 0xwilliamortiz/ratchet | 423 | 工具型 | 83 | 新增 |
| trycompai/crm | 3,123 | 平台候选 | 86 | 更新（1,731→3,123，+1,392） |
| microsoft/skill-recorder | 1,366 | 工具型 | 84 | 更新（726→1,366，+640） |
| sqliteai/waste | 1,520 | 观察型 | 83 | 更新（1,010→1,520，+510） |

#### 关键判断

- **qm 第三日增量反超第二日，脉冲假设彻底排除**：7,015→9,458（+2,443），而第二日是 +2,233。脉冲型热度的特征是尖峰后快速回落，不可能出现"衰减后回升"。三日连涨（+3,415/+2,233/+2,443）无一日回落，从 1,367 到 9,458，三日累计 +8,091 stars / +873 forks。应用层产品化从"持续性趋势"（08-03，两日支撑）升级为**完全确立的趋势**（08-04，三日支撑）。**证据边界**：star/fork 为可核验事实；"完全确立"为三日数据的强推断，第四日将进一步巩固。open_issues 113（较 08-03 的 85 上升），subscribers 47（相对 9.4K⭐ 偏低）。
- **K3 本地推理品类级爆发**：kimi-k3-in-c +955（218→1,173）与 waste +510（1,010→1,520）同步放量，两者增量量级相近，说明社区同时关注"可用速度"（waste）与"极限低内存"（kimi-k3-in-c）。注意一个传播偏向：kimi-k3-in-c 增量首次超过 waste，说明"最低 RAM"卖点传播力更强，但 32s/token 是"能跑"非"能用"。**证据边界**：8.24GB RSS 与字节一致输出仍为作者自述，fork 23→176 增长可能带来首批独立验证信号。
- **AI + 形式化验证交叉品类出现**：openai/ten-proofs 用 Lean 4 形式化十项数学成果，每个可 `lake build` 独立验证。这是 AI 推理↔形式化验证交叉点的高公信力信号（OpenAI 官方）。**证据边界**：Lean 编译通过只证明形式化内部自洽，formalization 与 informal claim 的对应需人工对照。
- **agent 质量基础设施成型**：ratchet（423⭐/85 fork/285 watchers）把 agent 极简规则从开环注入变闭环检查，与 skill-recorder 正交互补。285 watchers（>stars）的异常比例说明是深度关注而非浅层 star。**证据边界**：六类探测器为启发式判断，误报率未经公开评测，强绑定 Claude Code。
...
### 本地优先 AI Coworker 交付成品范式 · Agent RL 训练 Firecracker 基础设施开源 · TS→Native 零运行时编译 · 单文件自包含 AI 可编辑文档

#### 今日五大趋势

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | 本地优先 AI Coworker：OpenWorker 9.9K⭐（Andrew Ng/交付成品非聊天/审批门控/BYO 模型/25+ 连接器/aisuite），重定义 Agent 应用层信任模型 | openworker | 92 |
| 2 | Agent RL 训练基础设施开源：AgentENV 1.4K⭐（Rust/Firecracker microVM/快照 fork<50ms 恢复/为 Kimi K3 训练提供底座），环境即快照即 fork | agentenv | 90 |
| 3 | 零运行时语言编译：scriptc 2.1K⭐（Vercel Labs/TS→原生二进制/无 Node V8/178KB 2ms 启动/99% 静态），Agent 代码部署成本降维 | scriptc | 86 |
| 4 | 单文件自包含 + AI 可编辑 JSON：Bento 2.8K⭐（文件即软件/自重写保存/CRDT+E2EE/JSON 可被 Agent 原地编辑），文档作为 Agent 交付物新形态 | bento | 84 |

#### 今日新增/更新项目

| 项目 | Stars | 分类 | Score | 变化 |
|------|-------|------|-------|------|
| andrewyng/openworker | 9,965 | 平台候选 | 92 | 新增 |
| kvcache-ai/AgentENV | 1,411 | 基础设施候选 | 90 | 新增 |
| vercel-labs/scriptc | 2,099 | 工具型 | 86 | 新增 |
| nyblnet/bento | 2,806 | 工具型 | 84 | 新增 |

#### 关键判断

- **Agent 基础设施栈从「能跑」走向「可大规模训练、可交付成品」**：四个项目分别从应用层（OpenWorker）、训练层（AgentENV）、运行时层（scriptc）、文档层（Bento）共同验证 Agent 闭环交付与底层运行时工程化。
- **AgentENV 是今日最具基础设施价值的项目**：Firecracker microVM + 快照 fork 语义为 Agent RL 训练提供正确抽象，开源降低头部实验室训练管线复现门槛。风险：无鉴权、强依赖 KVM，生产化需时日。
- **scriptc 的「显式静态度」思想值得吸收**：逐构造决定编译边界、永不静默错误编译——对 Agent 生成代码的可审计性有启发。
...
| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | Agent Skill 从手工编写进化为可训练优化单元 | skillopt, html-anything, gsd-core | 92 |
| 2 | 自托管 AI 工作空间现象级项目 | odysseus | 90 |
| 3 | Agent 专用语言与运行时涌现 | zerolang, zero-native, forkd, rmux | 88 |
| 4 | AI 供应链安全成新赛道 | bumblebee, dirtyfrag | 84 |
| 5 | 开源情报平台与工具增强型项目持续走热 | osiris, codexplusplus | 80 |

#### 新增项目

| 项目 | Stars | 分类 | 首次出现 |
|------|-------|------|----------|
| skillopt | ~4.9K | 平台候选 | 2026-06-05 |
| odysseus | ~50.6K | 观察型 | 2026-06-05 |
| zerolang | ~4.9K | 学习型 | 2026-06-05 |
| bumblebee | ~4.3K | 工具型 | 2026-06-05 |
| forkd | ~1.3K | 基础设施候选 | 2026-06-05 |

#### 持续跟踪项目更新

| 项目 | 上次 stars | 本次 stars | 变化 |
|------|-----------|-----------|------|
| html-anything | 2.4K | 6.1K | +3.7K 持续上升 |
| mirage | 2.0K | 3.0K | +1.0K 稳定增长 |
| opensquilla | 893 | 2.9K | +2K 高增长 |
| ds4 | 8.6K | 12.9K | +4.3K 持续高热 |
...
> ⚠️ 外部数据源自 05-24 起连续 5 天不可达，本版基于 05-19~05-23 已采集数据完成趋势合成分析。

#### 今日五大趋势

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | Agent Harness 格局锁定 | ECC, Hermes Agent, OpenCode | 90 |
| 2 | Skills 生态垂直分化 | 9arm-skills, Claude Plugins Official | 88 |
| 3 | 小模型 Agent 工程化 | SmallCode, HRM-Text | 85 |
| 4 | 安全 Agent 双线闭环 | audit, Bumblebee | 83 |
| 5 | 团队级 Agent 基础设施 | Centaur | 80 |

#### 更新项目（last_seen 更新）

| 项目 | Stars（推算） | 分类 | 最近出现 |
|------|-------------|------|----------|
| SmallCode | ~1.2K | 工具型 | 2026-05-28 |
| Centaur | ~364 | 基础设施候选 | 2026-05-28 |
| audit | ~433 | 工具型 | 2026-05-28 |
| HRM-Text | ~650 | 学习型 | 2026-05-28 |

#### 趋势判断修正

- Agent Harness 三强格局（ECC 188.5K/Hermes 163K/OpenCode 164K）基本锁定，新入局者窗口关闭
- Skills 从通用框架走向垂直分化，每个垂直领域将有 1-2 个头部 Skills
- 小模型 Agent 工程化是 05-23 新出现的趋势，代表端侧 Agent 从概念走向工程适配
...
| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | Dirty Frag Linux LPE 席卷安全圈 | Dirty Frag | 90 |
| 2 | Agent 原生语言 Zero 暴涨 74% | Zero | 87 |
| 3 | AI 伴侣引擎浮出水面 | grok-animus, trustclaw | 82 |
| 4 | 统一存储抽象加速 | files-sdk | 78 |
| 5 | HTML Agent 工具链分化 | html-anything | 76 |

#### 今日新增项目

| 项目 | Stars | 分类 | 评分 | 备注 |
|------|-------|------|------|------|
| grok-animus | 573⭐ | 工具型 | 75 | 持久化 AI 伴侣引擎 |
| trustclaw | 665⭐ | 工具型 | 73 | 自托管个人 AI Agent |

#### 持续跟踪项目状态

| 项目 | 上次 Stars | 本次 Stars | 变化 | 状态 |
|------|-----------|-----------|------|------|
| ds4 | 10,038 | 10,317 | +279 | ✅ 稳增 |
| Zero | 916 | 1,600 | +684 | 🔥 暴增 |
| Dirty Frag | 2,775 | 4,611 | +1,836 | 🔥 暴增 |
| Mirage | 2,312 | 2,350 | +38 | ➡️ 平稳 |
| TokenSpeed | 1,031 | 1,036 | +5 | ➡️ 平稳 |
| html-anything | 2,425 | 2,758 | +333 | ✅ 稳增 |
| files-sdk | 642 | 772 | +130 | ✅ 稳增 |
| Pixal3D | 504 | 921 | +417 | ✅ 强增 |
| OpenSquilla | 893 | 969 | +76 | ✅ 稳增 |

---

## 2026-05-17（第三十七版）

### 本地推理双雄对决 · Agent VFS 基础设施化 · Agent 原生语言 Zero 问世

#### 今日五大趋势

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | 本地推理双雄对决 | ds4, TokenSpeed | 92 |
| 2 | Agent VFS 基础设施化 | Mirage | 88 |
| 3 | Agent 原生语言 Zero | Zero (Vercel Labs) | 85 |
| 4 | AI 安全新物种 | AiSOC | 82 |
| 5 | Token 效率成 Agent 新战场 | OpenSquilla | 80 |

#### 今日新增项目

| 项目 | Stars | 分类 | 评分 | 备注 |
|------|-------|------|------|------|
| Zero (vercel-labs) | 916⭐ | 观察型 | 78 | Vercel 推出的 Agent 编程语言 |
| AiSOC | 971⭐ | 工具型 | 76 | 开源 AI 安全运营中心 |
| OpenSquilla | 893⭐ | 观察型 | 74 | Token 高效 Agent |
| html-anything | 2,425⭐ | 工具型 | 73 | Agentic HTML 编辑器 |

#### 持续跟踪项目状态

| 项目 | 上次 Stars | 本次 Stars | 变化 | 状态 |
|------|-----------|-----------|------|------|
| ds4 | ~8.6K | 10,038 | +1.4K | ✅ 强增 |
| Mirage | ~2K | 2,312 | +300 | ✅ 稳增 |
| TokenSpeed | ~800 | 1,031 | +200 | ✅ 增长 |
| Chrome DevTools MCP | ~38K | 39,771 | +1.7K | ✅ 强增 |
| DeepClaude | ~1.5K | 1,875 | +300 | ✅ 稳增 |
| Hermes Agent | ~149K | 153,324 | +4K | ✅ 稳增 |
...
**⚠️ 数据来源：** 基于 2026-05-14 实测数据推算，外部网络不可达

#### 今日五大趋势

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | Agent 生态整合期三巨头格局 | Hermes Agent, Browser Harness, Open Design | 90 |
| 2 | Spec-Driven Dev 范式变迁 | Spec Kit | 87 |
| 3 | 本地推理三赛道分化复盘 | ds4.c, Terax AI, Zero Native | 83 |
| 4 | 安全赛道平台化趋势 | YellowKey, Deepsec | 80 |
| 5 | 硬件生态开放 vs 封闭 | OrcaSlicer-bambulab | 76 |

#### 持续跟踪项目状态（推算）

| 项目 | 上次 Stars | 本次推算 Stars | 变化 | 状态 |
|------|-----------|---------------|------|------|
| Hermes Agent | 148,546 | ~149K | +~500 | ✅ 稳增 |
| Spec Kit | 98,266 | ~99K | +~700 | ✅ 稳增 |
| Browser Harness | 93,774 | ~94K | +~300 | → 趋稳 |
| Open Design | 39,300 | ~40K | +~700 | ✅ 巩固期 |
| ds4.c | 8,387 | ~8.6K | +~200 | ✅ 稳增 |
| Zero Native | 3,261 | ~3.5K | +~200 | ↗ 新增 |
| YellowKey | 927 | ~1.2K | +~300 | ↗ 增长 |
| OrcaSlicer-bambulab | 2,832 | ~3.5K | +~700 | → 趋稳 |

#### 关键判断

1. **Agent 赛道从百花齐放到头部整合：** 三巨头（Hermes/Browser Harness/Open Design）占据生态关键位置
2. **Spec-Driven Dev 不是工具而是范式变迁：** Spec Kit 逼近 100K 里程碑，开发流程正在从 prompt 走向 spec
3. **本地推理赛道分化为三层：** 原生层(ds4.c) / 终端层(Terax AI) / 框架层(Zero Native)，不直接竞争
4. **安全赛道从事件驱动到平台化：** DevSecOps + Agent 是下一个爆发方向
5. **硬件生态开放 vs 封闭成为长期主题：** OrcaSlicer 分叉模式将被更多硬件领域复制

#### 数据来源标注

- 本日所有 Stars 数据基于 05-14 实测数据推算
- 推算方法：基于近期日增量线性外推
- 网络恢复后将验证并修正
...
**✅ 数据来源：** GitHub API 搜索实测，断网 3 天后首次真实数据。

#### 今日五大趋势

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | Agent 工具链全面扩张 | Open Design, Browser Harness, Hermes Agent | 92 |
| 2 | GitHub 官方入场 Spec-Driven Dev | GitHub Spec Kit | 88 |
| 3 | GPT-Image-2 生态爆发 | awesome-gpt-image-2, guizang-ppt-skill | 84 |
| 4 | 本地推理赛道分化 | ds4.c, Terax AI, Zero Native | 82 |
| 5 | 安全赛道升温 | CVE-2026-31431, Deepsec | 78 |

#### 持续跟踪项目状态（实测）

| 项目 | 上次 Stars | 本次 Stars | 变化 | 状态 |
|------|-----------|-----------|------|------|
| Open Design | 37,929（推演） | 38,354（实测） | +425 | ✅ 增速趋稳 |
| Browser Harness | 8,441（上次实测） | 12,304（实测） | +3,863 | 🔥 爆发增长 |
| Hermes Agent | 84.3K（旧数据） | 146,768（实测） | +62K | 🔥 跳涨 |
| ds4.c | 7,765（推演） | 8,042（实测） | +277 | ✅ 稳增 |
| Deepsec | 2,356（推演） | 2,415（实测） | +59 | ✅ 稳增 |
| CC Switch | 68,099（推演） | 68,591（实测） | +492 | ✅ 稳增 |

#### 新增项目

| 项目 | Stars | 分类 | 判断 |
|------|-------|------|------|
| GitHub Spec Kit | 97,190 | 基础设施候选 | 高优先级跟踪 |
| Terax AI | 2,583 | 工具型 | 有限跟踪 |
| Zero Native | 2,893 | 观察型 | 有限跟踪 |

#### 推演可信度验证

| 项目 | 推演值 | 实测值 | 偏差 |
|------|--------|--------|------|
| Open Design | 37,929 | 38,354 | 1.1% |
| ds4.c | 7,765 | 8,042 | 3.6% |
| CC Switch | 68,099 | 68,591 | 0.7% |

**结论：** 3 天断网推演整体偏差 < 4%，模型可靠。

---

## 2026-05-12（第三十三版）

### 第三天断网推演（基于 05-03 ~ 05-10 数据）

**⚠️ 数据来源：** 外部数据源连续第三天不可达，本轮为 Star 数推演的最后有效轮次。

#### 今日四大趋势

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | Agent 筛选期拐点 | CC Switch, MemPalace, Open Design, Mirage | 86 |
| 2 | DeepSeek V4 推理三层架构 | ds4.c, TokenSpeed, TileKernels | 84 |
| 3 | 安全双范式企业评估窗口 | Deepsec, Harmonist | 81 |
| 4 | 推演可信度衰减警告 | — | 75 |

#### 持续跟踪项目状态（推演 — 最终轮次）

| 项目 | 最后实测 Stars | 今日 Stars（推演） | 可信度 |
|------|---------------|-------------------|--------|
| CC Switch | ~59K（05-07） | ~65K | 中 |
| MemPalace | ~50.6K（05-10） | ~53K | 中 |
| Open Design | ~33.9K（05-09） | ~38K | 低 |
| ds4.c | ~2.3K（05-09） | ~3.8K | 低 |
| Mirage | ~1.4K（05-09） | ~2.5K | 低 |
| Deepsec | ~1.9K（05-09） | ~2.8K | 中 |
| Harmonist | ~1.4K（05-09） | ~2.3K | 中 |
| TokenSpeed | ~621（05-08） | ~1.3K | 低 |

#### 关键判断

1. **筛选期拐点已到：** 三天无数据本身就是筛选窗口，恢复后看谁还在增长
2. **DeepSeek V4 三层架构成型：** Kernel + 引擎 + 应用三层分工明确
3. **安全双范式可做 PoC：** Deepsec 审计 + Harmonist 预防闭环
4. **本轮为最后推演轮次：** 如果 05-13 仍无数据，转纯分析模式

#### 判断修正
- 无新增修正，维持筛选期判断
- 新增：推演可信度衰减警告，下调所有推演可信度

---

## 2026-05-11（第三十二版）

### 连续断网趋势推演（基于 05-03 ~ 05-10 数据）

**⚠️ 数据来源：** 外部数据源连续第二天不可达，基于本地历史数据推演。

#### 今日四大趋势

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | Agent 基础设施层进入筛选期 | CC Switch, MemPalace, Open Design, Mirage | 88 |
| 2 | 推理引擎从工具走向生态 | ds4.c, TokenSpeed, TileKernels | 85 |
| 3 | Agent 安全双范式验证 | Deepsec, Harmonist | 82 |
| 4 | Open Design 增速趋稳观察 | Open Design | 80 |

#### 持续跟踪项目状态（推演）

| 项目 | 上次 Stars | 今日 Stars（推演） | 推演增量 | 可信度 |
|------|-----------|-------------------|---------|--------|
| CC Switch | ~59K | ~63K | +4K | 高 |
| MemPalace | ~51K | ~52K | +1K | 高 |
| Open Design | 33.9K（实测） | ~36K | +2.1K | 中 |
| ds4.c | 2.3K（实测） | ~3.2K | +0.9K | 中 |
| Mirage | 1.4K（实测） | ~2.1K | +0.7K | 中 |
| Deepsec | 1.9K（实测） | ~2.5K | +0.6K | 中 |
| Harmonist | 1.4K（实测） | ~2.0K | +0.6K | 中 |
| TokenSpeed | 621（实测） | ~1.1K | +0.5K | 低 |

#### 关键判断

1. **Agent 生态筛选期到来：** 各层 1-2 个标准候选已出线（桌面: CC Switch, Memory: MemPalace），其余层级仍在竞争
2. **推理引擎生态化：** DeepSeek V4 推理从各自为战变为工具链协作
3. **安全双范式闭环可落地：** Deepsec（审计）+ Harmonist（预防）互补
4. **Open Design 进入验证期：** 从爆量增长到平台粘性验证是关键转折
...
### 周度复盘（2026-05-03 ~ 2026-05-09）

**⚠️ 数据来源：** 外部数据源不可达，基于本地历史数据推演。

#### 本周五大趋势

| 排名 | 趋势方向 | 代表项目 | 趋势分 |
|------|----------|----------|--------|
| 1 | Agent 生态分层成型 | Open Design, CC Switch, Mirage, MemPalace | 90 |
| 2 | Open Design 平台确认（周增 22K） | Open Design, PPT 赛道 | 87 |
| 3 | 推理引擎三线竞速 | ds4.c, TokenSpeed, TileKernels | 84 |
| 4 | Linux Page-Cache 安全系统性风险 | Dirty Frag, Copy Fail | 85 |
| 5 | Agent 安全双范式确立 | Deepsec, Harmonist | 82 |

#### 持续跟踪项目状态（周度）

| 项目 | 上周 Stars | 本周 Stars（预估） | 周增量 | 状态 |
|------|-----------|-------------------|--------|------|
| CC Switch | ~57K | ~62K | +5K | 📈 稳步增长 |
| MemPalace | ~49K | ~51K | +2K | 📈 稳定 |
| Open Design | ~11K | ~35K | +24K | 🚀 爆发后趋稳 |
| ds4.c | 0 | ~2.8K | 新增 | 🚀 本周新项目 |
| Deepsec | ~1.2K | ~2.2K | +1K | 📈 Agent 安全审计 |
| Mirage | ~400 | ~1.8K | +1.4K | 📈 VFS 方向确认 |
| TokenSpeed | 0 | ~900 | 新增 | 🚀 本周新项目 |
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| Dirty Frag | 安全研究 | 2,775 stars（2天） | Linux 内核通用 LPE，Dirty Pipe 家族漏洞，确定性触发无竞态 | 短期关注补丁 |

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日 Stars | 变化 | 判断 |
|------|-----------|-----------|------|------|
| Open Design | 32,118 | 33,880（实测） | +1.8K（+5%） | 平台地位确认，增速稳定 |
| ds4.c | 456 | 2,312（实测） | +1.9K（+407%） | 3天5x增长，升级基础设施候选 |
| Deepsec | 1,638 | 1,864（实测） | +226（+14%） | Agent 安全审计持续增长 |
| Harmonist | 1,200 | 1,423（实测） | +223（+19%） | 多Agent 编排框架稳步增长 |
| Mirage | 925 | 1,401（实测） | +476（+52%） | Agent VFS 赛道热度验证 |
| Open Slide | 897 | 2,624（实测） | +1,727（+193%） | Agent 原生 PPT 框架爆发 |

### 趋势判断

1. **Linux 内核安全密集爆发**：Dirty Frag + Copy Fail 同周披露两个 9 年 LPE，Page-Cache bug class 系统性风险
2. **Agent Design 平台确认**：Open Design 33.8K，从工具进化到平台，11 天 0→33K
3. **本地推理引擎加速**：ds4.c 3天 2.3K，磁盘 KV Cache 理念获得关注
4. **Agent 安全双范式成熟**：Deepsec（审计）+ Harmonist（预防）形成闭环

### 判断修正
- ds4.c 从工具型升级为基础设施候选（增速和磁盘 KV Cache 创新获得验证）
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| Mirage | 基础设施候选 | 925 stars（2天） | AI Agent 统一虚拟文件系统，范式级创新 | 持续跟踪 |
| TokenSpeed | 基础设施候选 | 621 stars（2天） | 面向 Agentic Workload 的 LLM 推理引擎 | 持续跟踪 |
| ds4.c | 工具型 | 456 stars（2天） | antirez 的 DeepSeek V4 Flash Metal 专用推理引擎 | 有限跟踪 |
| SPECA | 平台候选 | 345 stars（6天） | 规约锚定安全审计框架，有学术论文支撑 | 持续跟踪 |

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日 Stars | 变化 | 判断 |
|------|-----------|-----------|------|------|
| Open Design | ~31K | 32,118（实测） | +1K（+3%） | 赛道领跑者确认，10天0→32K |
| Deepsec | ~1.2K | 1,638（实测） | +400（+37%） | 增速验证，安全 Agent 需求确认 |
| DBX | 773 | 1,102 | +329（+43%） | 轻量数据库客户端持续增长 |

### 趋势判断

1. **Agent VFS 统一抽象层出现**：Mirage 用文件系统语义统一 Agent 后端访问，与 MCP 构成范式竞争
2. **推理引擎赛道分化**：通用（vLLM）vs Agent 特化（TokenSpeed）vs 端侧专用（ds4.c），三条路线并存
3. **安全审计从代码模式到规约锚定**：SPECA 用 Spec-anchored 方法检测传统工具无法发现的漏洞
4. **Open Design 32K 确认赛道领跑**：赛道整合期信号明确

### 判断修正

| 项目 | 原判断 | 修正判断 | 理由 |
|------|--------|---------|------|
| DBX | 工具型（score 74） | 工具型（score 72） | 轻量级工具定位，无平台化趋势，维持工具型 |
...
> ⚠️ 今日 GitHub 不可达，基于历史数据趋势分析，无新增项目。

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日预估 Stars | 变化 | 判断 |
|------|-----------|--------------|------|------|
| Open Design | 27.3K | ~31K | +3.7K（+14%） | 赛道进入整合期，从爆量转向筛选 |
| CC Switch | 57.8K | ~59K | +1.2K（+2%） | 生态锁定效应初显，Desktop 层事实标准 |
| PPT Master | 11.6K | ~12K | +400（+3%） | 稳定增长，PPT 赛道工业化代表 |
| Future AGI | 822 | ~900 | +78（+9%） | 上调至持续跟踪，可观测性赛道确定性高 |
| Deepsec | 1K | ~1.2K | +200（+20%） | 安全 Agent 化方向确认，产品早期 |
| Graphify | 39.6K | ~41K | +1.4K（+4%） | 上调至基础设施候选，Skill 生态基座 |

### 趋势判断

1. **Agent 生态分层已成定局**：Desktop / Runtime / Skills / Backend / Memory 五层架构清晰，每层开始出现事实标准
2. **赛道从爆量转向筛选**：Agent Design 赛道同质化项目将被淘汰，平台级能力是核心壁垒
3. **Future AGI 上调至持续跟踪**：Agent 可观测性是企业 Agent 生产化的必要条件，赛道确定性高
4. **Graphify 上调至基础设施候选**：Skill 跨平台标准化正在成为共识

### 判断修正

| 项目 | 原判断 | 修正判断 | 理由 |
|------|--------|---------|------|
| Future AGI | 有限跟踪 | 持续跟踪 | Agent 可观测性赛道窗口期，确定性强 |
| Graphify | 工具型 | 基础设施候选 | Skill 生态基座地位确认，日增460稳定 |
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| TileKernels | 基础设施候选 | 1.4K stars（12天） | DeepSeek GPU 算子库，MoE/量化/Engram 全覆盖 | 持续跟踪 |
| OpenChronicle | 基础设施候选 | 2.1K stars（13天） | OpenAI Chronicle 开源替代，本地优先 Agent Memory | 持续跟踪 |
| mhr-cfw | 工具型 | 1.9K stars（7天） | Domain Fronting DPI 绕过方案 | 有限跟踪 |
| text-to-cad | 工具型 | 1.4K stars（12天） | 开源 Text-to-CAD 引擎，AI + 工程设计 | 持续跟踪 |

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日 Stars | 变化 | 判断 |
|------|-----------|-----------|------|------|
| Open Design | 15.7K | 19.1K | +3.4K（+22%） | 持续爆发，增速放缓，赛道红海化 |
| Copy Fail CVE-2026-31431 | 1.6K | 3.0K | +1.4K（+88%） | 安全事件持续发酵，衍生项目涌现 |
| Harmonist | 945 | 1.2K | +28% | 稳步增长，零依赖方案获认可 |
| dbx | 707 | 773 | +9% | 稳定增长，轻量数据库客户端赛道 |

### 本日新增项目（2026-05-03）

| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| Club 3090 | 工具型 | 354 stars（5天） | RTX 3090 社区 LLM 部署 recipe 集，多引擎支持 | 有限跟踪 |
| OpenLess | 工具型 | 345 stars（6天） | 开源语音输入，按住说话松开出文 | 有限跟踪 |

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日 Stars | 变化 | 判断 |
|------|-----------|-----------|------|------|
| Open Design | 11.4K | 15.7K | +4.3K（+38%） | 持续爆发，5天4倍 |
| CC Switch | 44.7K | 57.8K | +13.1K（+29%/月） | 稳定增长，升级平台候选 |
| MemPalace | 50.7K | 50.7K | +0 | 稳守50K+，MCP修复 |
| dbx | 391 | 707 | +316（+81%） | 5天翻倍，SQL增强 |

### 趋势判断修正

- **重大修正**：CC Switch 从工具型升级为平台候选 — 57K+ stars + MCP + Skills 管理正在成为 Agent 生态的统一桌面入口
- **延续判断**：Open Design 持续爆发至 15.7K，Agent Design 赛道从概念进入产品化阶段
- **新增判断**：消费级 GPU LLM 部署（Club 3090）代表部署知识 recipe 化的趋势
- **新增判断**：AI 语音输入（OpenLess）开辟新赛道，但护城河浅
- **延续判断**：AI Memory 赛道 MemPalace 进入平台期，等待新增长驱动
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| HyperFrames | 工具型 | 13.6K stars | HeyGen 出品，HTML→Video Agent 原生视频生成 | 关注范式，非项目本身 |
| QuipNetwork | 观察型（疑似刷星） | 5.8K stars | Star/Fork 比异常（200:1），无代码更新 | **不建议跟踪** |

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日 Stars | 变化 | 判断 |
|------|-----------|-----------|------|------|
| Open Design | 8.0K | 11.4K | +3.4K（+43%） | 持续爆发，4天翻倍 |
| Graphify | 37.3K | 39.6K | +2.3K | 逼近40K，日增460 |
| MemPalace | 50.2K | 50.7K | +0.5K | 稳守50K+ |
| Obscura | 7.6K | 9.3K | +1.7K | 突破9K |
| Browser Harness | 8.4K | 9.2K | +0.8K | 稳步增长 |
| Chrome DevTools MCP | 35.0K | 37.9K | +2.9K | 持续高速增长 |
| Claw Code | 189.5K | 189.5K | +0 | 高位稳定 |

### 趋势判断修正

- **重大判断**：Agent Design 赛道进入爆发期 — Open Design 4天翻倍证明 Vibe Design 不只是概念，而是真实需求
- **重大判断**：Graphify 接近40K，跨 Agent 知识图谱正在成为 Agent 标准配置
- **新增判断**：Agent 原生媒体生成（HyperFrames）代表新范式，但商业化依赖是核心风险
- **新增判断**：QuipNetwork 系列疑似刷星，Star/Fork 比异常 + 零更新是甄别信号
- **延续判断**：AI Memory 赛道 MemPalace 进入平台期，等待新增长驱动
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| Copy Fail CVE-2026-31431 | 安全 | 1.6K stars（1天） | Linux 内核 AEAD 本地提权，9年历史漏洞，P0 安全事件 | **紧急关注** |
| FreeLLMAPI | 工具型 | 783 stars（10天） | 聚合14家免费 LLM Provider，月1.3B+ tokens | 合规观察 |
| World2Agent | 基础设施候选 | 588 stars（8天） | Agent 感知层开放协议，Sensor → Agent 标准化 | 跟踪 |
| dbx | 工具型 | 391 stars（2天） | Tauri+Rust 轻量级数据库客户端，支持8种DB | 低优先级跟踪 |

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日 Stars | 变化 | 判断 |
|------|-----------|-----------|------|------|
| Open Design | 4.1K | 8.0K | +3.9K（+95%） | 爆发增长，升级为平台候选 |
| OpenMythos | 11.2K | 11.2K | +0 | 稳定 |
| OpenChronicle | 1.9K | 1.9K | +0 | 稳定 |
| Harmonist | 893 | 945 | +52 | 稳步增长，升级为平台候选 |
| TileKernels | 1.3K | 1.4K | +0.1K | 稳定 |

### 趋势判断修正

- **重大修正**：Open Design 从工具型升级为平台候选 — 72 Design Systems + 31 Skills + 10 Agent CLI 支持，正在整合 Agent Design 赛道
- **重大修正**：Harmonist 从观察型升级为平台候选 — 协议硬强制模式是 Agent 安全层的重要创新
- **新增判断**：Linux 内核 CVE-2026-31431 是2026年迄今最严重的内核安全漏洞，需紧急关注补丁状态
- **新增判断**：GPU Kernel 层开源（DeepSeek TileKernels）标志着大模型竞争下沉到算子层
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| Hy3 Preview | 观察型 | 262 stars（刚发布） | 腾讯混元 295B MoE，90天重建基础设施 | 持续跟踪，等 GA |
| Kimi K2.6 | 基础设施候选 | 10.7K stars | SWE-Bench Pro 58.6% 登顶，300 Agent Swarm | **PoC 推荐** |
| Harmonist | 观察型 | 741 stars | 零依赖 Agent 编排，186 Agent | 低优先级跟踪 |
...
| GitNexus | 25K | 28.6K | +3.6K | 持续增长 |
| huashu-design | 4.4K | 8.3K | +3.9K | 快速增长 |
| Ollama | 168K | 170.2K | +2.2K | 稳定 |

### 趋势判断修正

- **新增判断**：MoE 已成为国产大模型的标准架构选择，Hy3 和 Kimi K2.6 双雄并立验证了这一趋势
- **新增判断**：Agent Swarm（多 Agent 并行编排）可能是 Agentic Coding 的下一个范式突破
- **维持判断**：MCP 协议正在成为 Agent 工具链的标准接口
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| PgQue | **基础设施候选** | 913 stars, 4天 | 极简 Postgres 队列，适度架构思潮 | **持续跟踪** |
| Kami | 工具型 | 1354 stars, 2天 | tw93 出品，内容排版工具 | 短期观望 |
| diagram-design | 工具型 | 1303 stars, 6天 | Claude Code Skill 图表设计 | 短期观望 |

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日 Stars | 变化 | 判断 |
|------|-----------|-----------|------|------|
| browser-harness | 3,458 | 4,347 | +889 | 持续爆发，升级为平台候选 |
| codeburn | 2,553 | 3,239 | +686 | AI 编码可观测性刚需验证 |
| OpenMythos | 3,995 | 6,615 | +2,620 | 持续爆发，理论项目罕见 |
| agentic-stack | 750 | 1,231 | +481 | 增速加快 |
| anything-analyzer | 1,200 | 1,607 | +407 | 稳定增长 |
| lingbot-map | 3,194 | 3,860 | +666 | 持续增长 |
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| OpenMythos | **学习型** | 3995 stars, 3天 | Claude 架构逆向重建，学术价值高 | **持续跟踪** |
| aube | 工具型 | 318 stars, 3天 | Rust 实现的 Node.js 包管理器 | 短期观望 |

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日 Stars | 变化 | 判断 |
|------|-----------|-----------|------|------|
| browser-harness | 2,031 | 3,458 | +1,427 | 增速爆发，成为 Browser Agent 标杆 |
| agentic-stack | 570 | 750 | +180 | 持续增长，升级为平台候选 |
| weft | 824 | 906 | +82 | 稳定增长 |
| design-extract | 1,084 | 1,138 | +54 | 稳定增长 |
| lingbot-map | 2,596 | 3,194 | +598 | 增速强劲 |
| xata | 634 | 680 | +46 | 稳定增长 |
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| browser-harness | **工具型** | 2031 stars, 4天 | Self-healing browser harness，CDP 裸协议 | **持续跟踪** |
| design-extract | 工具型 | 1084 stars, 5天 | 设计系统提取 + MCP server + 多平台 emit | **持续跟踪** |
| html-ppt-skill | 工具型 | 1396 stars, 5天 | HTML PPT AgentSkill | 短期关注 |

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日 Stars | 变化 | 判断 |
|------|-----------|-----------|------|------|
| wterm | 1,310 | 2,050 | +740 | Vercel 品牌效应，增速加快 |
| xata | 479 | 634 | +155 | 稳定增长，基础设施定位清晰 |
| lingbot-map | 1,778 | 2,596 | +818 | 增速强劲，3D 赛道关注度提升 |
| agentic-stack | 492 | 570 | +78 | 持续增长 |
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| agentic-stack | **工具型** | 492 stars, 4天 | Agent 可移植性层，.agent/ 跨 harness | **持续跟踪** |
| meta-harness | 学习型 | 444 stars, 4天 | Stanford 出品，Harness 自动优化 | **持续跟踪** |
| openduck | 工具型 | 447 stars, 5天 | 分布式 DuckDB，双执行引擎 | **持续跟踪** |
| lingbot-map | 学习型 | 1,778 stars, 4天 | 前馈 3D 基础模型，streaming 场景重建 | 观察 |

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日 Stars | 变化 | 判断 |
|------|-----------|-----------|------|------|
| codeburn | 2,553 | 2,700 | +147 | 增速放缓但仍健康 |
| weft | 769 | 824 | +55 | 持续增长，方向正确 |
| anything-analyzer | 1,269 | 1,346 | +77 | 稳定增长 |
| darwin-skill | 1,082 | 1,189 | +107 | 稳定增长 |
| wterm | 1,310 | 1,457 | +147 | 持续高速增长 |
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| wterm | **基础设施候选** | 1,310 stars, 4天 | Vercel 出品 Web 终端，Zig+WASM+DOM | **持续跟踪** |
| anything-analyzer | 工具型/平台候选 | 1,269 stars, 5天 | 全场景协议分析+AI逆向+MCP | **持续跟踪** |
| video-use | 工具型 | 903 stars, 5天 | browser-use 出品视频编辑 Skill | 短期跟踪 |

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日 Stars | 变化 | 判断 |
|------|-----------|-----------|------|------|
| codeburn | 2,214 | 2,553 | +339 | 持续高速增长，FinOps 潜力 |
| darwin-skill | 952 | 1,082 | +130 | 稳定增长，但活跃度下降 |
| weft | 638 | 769 | +131 | 持续迭代，方向正确 |
| claw-code | 185,298 | 185,698 | +400 | 稳定 |
| autoagent | 4,160 | 4,160 | 0 | ⚠️ 建议降级为观察型 |

### 本日趋势信号

1. **Claude Code Skill 生态爆发**：video-use、design-extract、html-ppt-skill 集中涌现，Skill 从开发工具延伸到内容生产
2. **MCP 作为工具链统一接口**：anything-analyzer 验证了 MCP 在安全/逆向场景的价值
3. **Web 终端基础设施加速**：wterm 用 Zig+WASM 挑战 xterm.js
4. **Token 可观测性平台化**：codeburn 增速稳定，企业 FinOps 需求显现
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| darwin-skill | **平台候选** | 952 stars, 4天 | Skill 自进化引擎：评估→改进→回滚 | **持续跟踪** |
| weft | 基础设施候选 | 638 stars | AI 原生编程语言，Rust 实现 | 长期观察 |
| parlor | 工具型 | 1,551 stars | 端侧多模态 AI 对话，Apple Silicon | 短期关注 |
| dflash-mlx | 工具型/基础设施候选 | 443 stars | MLX 无损推测解码 | 持续跟踪 |
| xata | 基础设施候选 | 479 stars | 开源云原生 Postgres，CoW 分支 | 短期关注 |

### 持续跟踪项目状态更新

| 项目 | 上次 Stars | 今日 Stars | 变化 | 判断 |
|------|-----------|-----------|------|------|
| mempalace | 46,544 | 47,100 | +556 | 稳定增长，AI 记忆系统赛道领跑 |
| codeburn | 1,698 | 2,214 | +516 | Token 可观测性持续升温 |
| claw-code | 184,298 | 185,298 | +1,000 | 稳定 |
| graphify | 28,012 | 28,212 | +200 | 稳定 |
| nuwa-skill | 11,449 | 11,749 | +300 | 稳定 |
| autoagent | 4,160 | 4,160 | 0 | ⚠️ 两周不活跃 |

### 本日趋势信号

1. **Skill 自进化成为新范式**：darwin-skill 的评估-改进-回滚闭环，标志着 Skill 从静态模板走向达尔文式迭代
2. **AI 原生编程语言登场**：weft（Rust）探索为 Agent 系统设计编译层，方向前沿
3. **端侧推理加速**：dflash-mlx（推测解码）+ parlor（多模态对话），Apple Silicon 生态加速
4. **⚠️ QuipNetwork 系列存疑**：四个仓库同时 ~4.4K stars，疑似 star farming
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| mempalace | **基础设施候选** | 41,755 stars, LongMemEval 96.6% | 史上最高分 AI 记忆系统，完全本地免费 | **深度跟踪 + PoC** |
| gbrain | 工具型/基础设施候选 | 15,432 stars | Agent 知识脑系统，知识图谱 + 记忆融合 | 持续跟踪 |
| graphify | 工具型 | 22,086 stars, 9天2万+ | 代码/文档转知识图谱 Agent Skill | 持续跟踪 |
| clicky | 工具型 | 3,774 stars | 光标旁 AI 老师，Screen AI + Voice AI | 观望 |

### 趋势信号更新

- **AI 记忆系统破局**：mempalace 在 LongMemEval 达到 96.6%，AI 记忆从概念进入实用阶段
- **Agent 知识脑标配化**：gbrain + graphify 代表 Agent 对知识感知的需求爆发
- **人格蒸馏 Skill 生态**：nuwa-skill 等项目显示 AI 人格可迁移成为新方向
...
| 项目 | 分类 | 热度信号 | 核心判断 | 建议 |
|------|------|---------|---------|------|
| litert-inference | **基础设施候选** | 2341 stars, Google官方 | 生产级端侧LLM推理引擎，TFLite继任者 | **深度跟踪 + PoC** |
| claude-code-extension | **平台候选** | 8942 stars, 官方扩展 | Claude Code扩展生态，官方插件和工具 | 持续跟踪 |
| agent-orchestra | 工具型 | 678 stars | 多代理协作编排平台，类似交响乐协作模式 | 持续跟踪 |
| gemma-4-mobile | 工具型/基础设施候选 | 1892 stars | Gemma 4移动端优化，轻量级大模型 | 关注端侧赛道 |
| oh-my-claudecode-v2 | 平台候选 | 12156 stars | Multi-Agent团队化编排v2 | 持续跟踪 |
| skill-chain | 工具型 | 543 stars | Agent技能链管理系统，技能组合复用 | 观察为主 |
| memory-palace | 工具型 | 432 stars | Agent长期记忆系统，跨会话知识积累 | 观察为主 |
| devops-ai | 工具型 | 789 stars | AI驱动DevOps自动化，全流程智能化 | 观察为主 |
| coderaft-pro | 工具型 | 156 stars | 轻量级远程开发环境专业版 | 观察为主 |
| on-device-ml | 工具型 | 3256 stars | 跨平台端侧ML开发框架 | 持续跟踪 |

### 持续跟踪项目（无重大变化）

| 项目 | 分类 | 热度信号 | 核心判断 | 状态 |
|------|------|---------|---------|------|
| hermes-agent | 基础设施候选 | 9662/week, 28k stars | Skill自进化 + 多会话记忆，触及Agent核心问题 | **深度跟踪 + PoC** |
| oh-my-claudecode | 平台候选 | 7832/week, 25k stars | 团队化Multi-Agent编排，Claude Code生态 | 持续跟踪 |
| VibeVoice | 生产可用 | 8392/week, 37k stars | 微软Voice AI，ASR已进HF Transformers | 关注ASR部分 |
| shannon | 泡沫型/学习型 | 733/day(?), 37k stars | 白盒渗透测试，真实攻击噱头大于实用 | 不建议企业落地 |
| openscreen | 工具型 | 14396/week, 24k stars | 开源Screen Studio替代，录制工具 | 轻量级分享工具定位 |
| oh-my-codex | 工具型 | 14247/week, 18k stars | oh-my-claudecode的Codex版本 | 关注与claude版本的融合可能 |
| auto-deep-researcher-24x7 | 基础设施候选 | 123 stars (2天前) | 24/7自动深度研究系统 | 持续跟踪 |
| styleseed | 工具型 | 120 stars (2天前) | AI设计引擎，让AI代码像UI/UX设计师 | 持续跟踪 |
| project-architect | 工具型 | 94 stars (2天前) | 文档优先的项目规划代理技能 | 持续跟踪 |
| GitNexus | 工具型/基础设施候选 | 857/day, 23.6k stars | 代码知识图谱 + MCP Server，AI Coding Agent感知层 | 值得深挖 |
| qmd | 工具型 | 394/day, 18.8k stars | 本地文档搜索BM25+向量+LLaMA.cpp，MCP暴露 | 持续跟踪 |
| LiteRT-LM | 基础设施候选 | 483/day, 2k stars | Google生产级端侧LLM推理，已在Chrome/Gemini等落地 | 关注Google AI Edge生态 |
...
### 端侧AI推理生产化（新趋势）
- litert-inference（Google官方，2341 stars）
- gemma-4-mobile（Google官方，1892 stars）
- on-device-ml（TensorFlow生态，3256 stars）

**判断：** 端侧AI推理从概念验证走向生产落地，Google引领端侧推理革命，真正的生产级推理框架出现。

### AI编程平台生态扩张（持续）
- claude-code-extension（官方扩展，8942 stars）
- oh-my-claudecode-v2（团队化编排v2，12156 stars）
- oh-my-codex（Codex版本，14247/week）

**判断：** AI编程平台生态开始成熟，从单一工具向完整平台演进，扩展机制和插件生态成为关键。

### 多代理协作标准化（新信号）
- agent-orchestra（交响乐协作模式，678 stars）
- skill-chain（技能链管理，543 stars）
- memory-palace（长期记忆系统，432 stars）

**判断：** 多代理协作从简单协作向复杂编排演进，协作模式开始标准化，类似音乐编排的抽象模式出现。

### Claude Code生态繁荣（持续）
- claude-code-extension（官方扩展，8942 stars）
- oh-my-claudecode（团队化编排，7832/week）
- oh-my-claudecode-v2（v2版本，12156 stars）

**判断：** Claude Code正在成为AI编程的标准平台，其扩展生态繁荣，官方和第三方扩展形成完整生态。
...
| 项目 | 上期排名 | 本期 | 变化 |
|------|---------|------|------|
| litert-inference | - | 新晋热榜 | 新增 |
| claude-code-extension | - | 新晋热榜 | 新增 |
| agent-orchestra | - | 新晋热榜 | 新增 |
| gemma-4-mobile | - | 新晋热榜 | 新增 |
| oh-my-claudecode-v2 | - | 新晋热榜 | 新增 |
| skill-chain | - | 新晋热榜 | 新增 |
| memory-palace | - | 新晋热榜 | 新增 |
| devops-ai | - | 新晋热榜 | 新增 |
| coderaft-pro | - | 新晋热榜 | 新增 |
| on-device-ml | - | 新晋热榜 | 新增 |

---

## 关键判断修正

### 修正：端侧AI推理从"学习型"升级为"基础设施候选"
- **原判断**：LiteRT-LM 为学习型，了解端侧AI能力边界
- **修正判断**：端侧AI推理已进入生产化阶段，litert-inference 代表真正的生产级基础设施
- **原因**：Google大规模生产落地，端云协同架构成熟，企业级应用案例出现

### 修正：Claude Code生态从"持续跟踪"升级为"平台候选"
- **原判断**：oh-my-claudecode 为工具型，持续跟踪
- **修正判断**：Claude Code 生态已形成平台级生态，正式升级为平台候选
- **原因**：官方扩展机制确立，第三方扩展繁荣，用户基数快速增长

### 新判断：多代理协作范式开始形成
- **趋势**：agent-orchestra 等项目显示协作模式开始标准化
- **判断**：从简单协作向复杂编排演进，协作范式形成
- **意义**：多Agent系统从概念走向实际工程化应用
